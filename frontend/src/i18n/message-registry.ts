export const MESSAGE_REGISTRY = {
  "components.WebhookComponent": {
    "enable": {
      "en": "Enable",
      "zh": "启用"
    },
    "fillInDemoTip": {
      "en": "Please modify the URL and other settings to your own",
      "zh": "请修改URL和其他设置为您自己的配置"
    },
    "notEnabled": {
      "en": "Webhook is not enabled for you",
      "zh": "Webhook 未开启，请联系管理员开启"
    },
    "presets": {
      "en": "Presets",
      "zh": "示例模板"
    },
    "save": {
      "en": "Save",
      "zh": "保存"
    },
    "successTip": {
      "en": "Success",
      "zh": "成功"
    },
    "test": {
      "en": "Test",
      "zh": "测试"
    },
    "urlMissing": {
      "en": "URL is required",
      "zh": "URL 不能为空"
    }
  },
  "components.MailBox": {
    "attachments": {
      "en": "Show Attachments",
      "zh": "查看附件"
    },
    "autoRefresh": {
      "en": "Auto Refresh",
      "zh": "自动刷新"
    },
    "backToList": {
      "en": "Back to List",
      "zh": "返回列表"
    },
    "cancelMultiAction": {
      "en": "Cancel Multi Action",
      "zh": "取消多选"
    },
    "delete": {
      "en": "Delete",
      "zh": "删除"
    },
    "deleteMailTip": {
      "en": "Are you sure you want to delete mail?",
      "zh": "确定要删除邮件吗?"
    },
    "downloadMail": {
      "en": "Download Mail",
      "zh": "下载邮件"
    },
    "emptyInbox": {
      "en": "Your inbox is empty",
      "zh": "收件箱为空"
    },
    "forwardMail": {
      "en": "Forward",
      "zh": "转发"
    },
    "keywordQueryTip": {
      "en": "Filter current page",
      "zh": "过滤当前页"
    },
    "multiAction": {
      "en": "Multi Action",
      "zh": "多选"
    },
    "nextMail": {
      "en": "Next",
      "zh": "下一封"
    },
    "pleaseSelectMail": {
      "en": "Please select mail",
      "zh": "请选择邮件"
    },
    "prevMail": {
      "en": "Previous",
      "zh": "上一封"
    },
    "query": {
      "en": "Query",
      "zh": "查询"
    },
    "refresh": {
      "en": "Refresh",
      "zh": "刷新"
    },
    "refreshAfter": {
      "en": "Refresh After {msg} Seconds",
      "zh": "{msg}秒后刷新"
    },
    "reply": {
      "en": "Reply",
      "zh": "回复"
    },
    "saveToS3": {
      "en": "Save to S3",
      "zh": "保存到S3"
    },
    "selectAll": {
      "en": "Select All of This Page",
      "zh": "全选本页"
    },
    "showHtmlMail": {
      "en": "Show Html Mail",
      "zh": "显示HTML邮件"
    },
    "showTextMail": {
      "en": "Show Text Mail",
      "zh": "显示纯文本邮件"
    },
    "success": {
      "en": "Success",
      "zh": "成功"
    },
    "unselectAll": {
      "en": "Unselect All",
      "zh": "取消全选"
    }
  },
  "components.AiExtractInfo": {
    "authCode": {
      "en": "Verification Code",
      "zh": "验证码"
    },
    "authLink": {
      "en": "Authentication Link",
      "zh": "认证链接"
    },
    "copyFailed": {
      "en": "Copy failed",
      "zh": "复制失败"
    },
    "copySuccess": {
      "en": "Copied successfully",
      "zh": "复制成功"
    },
    "open": {
      "en": "Open",
      "zh": "打开"
    },
    "otherLink": {
      "en": "Other Link",
      "zh": "其他链接"
    },
    "serviceLink": {
      "en": "Service Link",
      "zh": "服务链接"
    },
    "subscriptionLink": {
      "en": "Subscription Link",
      "zh": "订阅链接"
    }
  },
  "components.MailContentRenderer": {
    "attachments": {
      "en": "View Attachments",
      "zh": "查看附件"
    },
    "delete": {
      "en": "Delete",
      "zh": "删除"
    },
    "deleteMailTip": {
      "en": "Are you sure you want to delete mail?",
      "zh": "确定要删除邮件吗?"
    },
    "downloadMail": {
      "en": "Download Mail",
      "zh": "下载邮件"
    },
    "forward": {
      "en": "Forward",
      "zh": "转发"
    },
    "fullscreen": {
      "en": "Fullscreen",
      "zh": "全屏"
    },
    "loadRemoteImages": {
      "en": "Load Images",
      "zh": "加载图片"
    },
    "remoteImagesBlocked": {
      "en": "{count} remote resources blocked to protect your privacy",
      "zh": "已阻止 {count} 项外部资源以保护隐私"
    },
    "reply": {
      "en": "Reply",
      "zh": "回复"
    },
    "saveToS3": {
      "en": "Save to S3",
      "zh": "保存到S3"
    },
    "showHtmlMail": {
      "en": "Show HTML Mail",
      "zh": "显示HTML邮件"
    },
    "showTextMail": {
      "en": "Show Text Mail",
      "zh": "显示纯文本邮件"
    },
    "size": {
      "en": "Size",
      "zh": "大小"
    }
  },
  "components.Turnstile": {
    "refresh": {
      "en": "Refresh",
      "zh": "刷新"
    }
  },
  "components.SendBox": {
    "cancelMultiAction": {
      "en": "Cancel Multi Action",
      "zh": "取消多选"
    },
    "delete": {
      "en": "Delete",
      "zh": "删除"
    },
    "deleteMailTip": {
      "en": "Are you sure you want to delete mail?",
      "zh": "确定要删除邮件吗?"
    },
    "emptySent": {
      "en": "No sent emails",
      "zh": "发件箱为空"
    },
    "multiAction": {
      "en": "Multi Action",
      "zh": "多选"
    },
    "pleaseSelectMail": {
      "en": "Please select a mail to view.",
      "zh": "请选择一封邮件查看。"
    },
    "refresh": {
      "en": "Refresh",
      "zh": "刷新"
    },
    "selectAll": {
      "en": "Select All of This Page",
      "zh": "全选本页"
    },
    "showCode": {
      "en": "Change View Original Code",
      "zh": "切换查看元数据"
    },
    "success": {
      "en": "Success",
      "zh": "成功"
    },
    "unselectAll": {
      "en": "Unselect All",
      "zh": "取消全选"
    }
  },
  "components.AddressSelect": {
    "address": {
      "en": "Address",
      "zh": "地址"
    },
    "copied": {
      "en": "Copied",
      "zh": "已复制"
    },
    "copy": {
      "en": "Copy",
      "zh": "复制"
    },
    "localAddresses": {
      "en": "Local Addresses",
      "zh": "本地地址"
    },
    "userAddresses": {
      "en": "User Addresses",
      "zh": "用户地址"
    }
  },
  "components.AddressCredentialModal": {
    "addressCredential": {
      "en": "Address Credential",
      "zh": "地址凭证"
    },
    "addressCredentialLabel": {
      "en": "Address JWT",
      "zh": "Address JWT"
    },
    "addressPassword": {
      "en": "Address Password",
      "zh": "地址密码"
    },
    "agentAccess": {
      "en": "AI Agent",
      "zh": "AI Agent"
    },
    "agentAccessTip": {
      "en": "Use this mailbox from an AI agent with the Address JWT and parsed-mail APIs.",
      "zh": "AI Agent 可使用 Address JWT 和 parsed-mail API 读取这个邮箱。"
    },
    "agentConfig": {
      "en": "Agent config",
      "zh": "Agent 配置"
    },
    "agentSkill": {
      "en": "Agent skill",
      "zh": "Agent skill"
    },
    "apiBase": {
      "en": "API Base",
      "zh": "API 地址"
    },
    "autoLoginLink": {
      "en": "Auto-login link",
      "zh": "自动登录链接"
    },
    "copyFailed": {
      "en": "Copy failed",
      "zh": "复制失败"
    },
    "copySection": {
      "en": "Copy",
      "zh": "复制"
    },
    "copySuccess": {
      "en": "Copied",
      "zh": "已复制"
    },
    "currentAddress": {
      "en": "Current address",
      "zh": "当前邮箱"
    },
    "docs": {
      "en": "Docs",
      "zh": "文档"
    },
    "imapHost": {
      "en": "IMAP host",
      "zh": "IMAP 主机"
    },
    "imapPort": {
      "en": "IMAP port",
      "zh": "IMAP 端口"
    },
    "password": {
      "en": "Password",
      "zh": "密码"
    },
    "plainOrProxyTls": {
      "en": "Plain or proxy TLS",
      "zh": "明文或代理层 TLS"
    },
    "security": {
      "en": "Security",
      "zh": "安全"
    },
    "smtpHost": {
      "en": "SMTP host",
      "zh": "SMTP 主机"
    },
    "smtpImapAccess": {
      "en": "SMTP / IMAP",
      "zh": "SMTP / IMAP"
    },
    "smtpImapTip": {
      "en": "Use these values in mail clients after the administrator configures the SMTP/IMAP proxy. The password can be the Address JWT shown here, or the address password when you have it.",
      "zh": "管理员配置 SMTP/IMAP 代理后，可在邮件客户端中使用这些信息。密码可使用这里展示的 Address JWT，也可使用你持有的地址密码。"
    },
    "smtpPort": {
      "en": "SMTP port",
      "zh": "SMTP 端口"
    },
    "starttls": {
      "en": "STARTTLS",
      "zh": "STARTTLS"
    },
    "tip": {
      "en": "Use these credentials only with clients and agents you trust.",
      "zh": "请只在可信的客户端和 Agent 中使用这些凭证。"
    },
    "title": {
      "en": "Address Credentials & Connection Methods",
      "zh": "地址凭证与连接方式"
    },
    "username": {
      "en": "Username",
      "zh": "用户名"
    }
  },
  "views.user.UserMailBox": {
    "addressQueryTip": {
      "en": "Leave blank to query all addresses",
      "zh": "留空查询所有地址"
    },
    "query": {
      "en": "Query",
      "zh": "查询"
    }
  },
  "views.Index": {
    "about": {
      "en": "About",
      "zh": "关于"
    },
    "accountSettings": {
      "en": "Email Settings",
      "zh": "邮箱设置"
    },
    "addressGroups": {
      "en": "Groups",
      "zh": "分组"
    },
    "appearance": {
      "en": "Appearance",
      "zh": "外观"
    },
    "auto_reply": {
      "en": "Auto Reply",
      "zh": "自动回复"
    },
    "enterSimpleMode": {
      "en": "Simple Mode",
      "zh": "极简模式"
    },
    "mailbox": {
      "en": "Mail Box",
      "zh": "收件箱"
    },
    "aiAdvisor": {
      "en": "AI Advisor",
      "zh": "AI 顾问"
    },
    "apiTokens": {
      "en": "API tokens",
      "zh": "API 令牌"
    },
    "query": {
      "en": "Query",
      "zh": "查询"
    },
    "s3Attachment": {
      "en": "S3 Attachment",
      "zh": "S3附件"
    },
    "saveToS3Success": {
      "en": "save to s3 success",
      "zh": "保存到s3成功"
    },
    "sendbox": {
      "en": "Send Box",
      "zh": "发件箱"
    },
    "sendmail": {
      "en": "Send Mail",
      "zh": "发送邮件"
    },
    "webhookSettings": {
      "en": "Webhook Settings",
      "zh": "Webhook 设置"
    }
  },
  "views.Footer": {
    "copyright": {
      "en": "版权所有 ©2026-2030 企业邮箱转发",
      "zh": "版权所有 ©2026-2030 企业邮箱转发"
    }
  },
  "views.user.UserSettings": {
    "actions": {
      "en": "Actions",
      "zh": "操作"
    },
    "createPasskey": {
      "en": "Create Passkey",
      "zh": "创建 Passkey"
    },
    "created_at": {
      "en": "Created At",
      "zh": "创建时间"
    },
    "deletePasskey": {
      "en": "Delete Passkey",
      "zh": "删除 Passkey"
    },
    "logout": {
      "en": "Logout",
      "zh": "退出登录"
    },
    "logoutConfirm": {
      "en": "Are you sure you want to logout?",
      "zh": "确定要退出登录吗？"
    },
    "passkeyCreated": {
      "en": "Passkey created successfully",
      "zh": "Passkey 创建成功"
    },
    "passkeyNamePlaceholder": {
      "en": "Please enter the passkey name or leave it empty to generate a random one",
      "zh": "请输入 Passkey 名称或者留空自动生成"
    },
    "passkey_name": {
      "en": "Passkey Name",
      "zh": "Passkey 名称"
    },
    "passordTip": {
      "en": "The server will only receive the hash value of the password, and will not receive the plaintext password, so it cannot view or retrieve your password. If the administrator enables email verification, you can reset the password in incognito mode",
      "zh": "服务器只会接收到密码的哈希值，不会接收到明文密码，因此无法查看或者找回您的密码, 如果管理员启用了邮件验证您可以在无痕模式重置密码"
    },
    "renamePasskey": {
      "en": "Rename Passkey",
      "zh": "重命名 Passkey"
    },
    "renamePasskeyNamePlaceholder": {
      "en": "Please enter the new passkey name",
      "zh": "请输入新的 Passkey 名称"
    },
    "showPasskeyList": {
      "en": "Show Passkey List",
      "zh": "查看 Passkey 列表"
    },
    "updated_at": {
      "en": "Updated At",
      "zh": "更新时间"
    }
  },
  "views.Header": {
    "accessHeader": {
      "en": "Access Password",
      "zh": "访问密码"
    },
    "accessTip": {
      "en": "Please enter the correct access password",
      "zh": "请输入站点访问密码"
    },
    "dark": {
      "en": "Dark",
      "zh": "暗色"
    },
    "home": {
      "en": "Home",
      "zh": "主页"
    },
    "light": {
      "en": "Light",
      "zh": "亮色"
    },
    "menu": {
      "en": "Menu",
      "zh": "菜单"
    },
    "ok": {
      "en": "OK",
      "zh": "确定"
    },
    "status": {
      "en": "Status",
      "zh": "状态"
    },
    "title": {
      "en": "企业邮箱管理平台",
      "zh": "企业邮箱管理平台"
    },
    "user": {
      "en": "User",
      "zh": "用户"
    }
  },
  "views.user.BindAddress": {
    "logout": {
      "en": "Logout",
      "zh": "退出登录"
    }
  },
  "views.Admin": {
    "about": {
      "en": "About",
      "zh": "关于"
    },
    "accessHeader": {
      "en": "Admin Password",
      "zh": "Admin 密码"
    },
    "accessTip": {
      "en": "Please enter the admin password",
      "zh": "请输入 Admin 密码"
    },
    "account": {
      "en": "Account",
      "zh": "账号"
    },
    "account_create": {
      "en": "Create Account",
      "zh": "创建账号"
    },
    "address_groups": {
      "en": "Groups",
      "zh": "分组"
    },
    "account_settings": {
      "en": "Account Settings",
      "zh": "账号设置"
    },
    "adminAccount": {
      "en": "Admin",
      "zh": "管理员"
    },
    "aiAdvisor": {
      "en": "AI Advisor",
      "zh": "AI 顾问"
    },
    "apiTokens": {
      "en": "API tokens",
      "zh": "API 令牌"
    },
    "appearance": {
      "en": "Appearance",
      "zh": "外观"
    },
    "confirm": {
      "en": "Confirm",
      "zh": "确认"
    },
    "database": {
      "en": "Database",
      "zh": "数据库"
    },
    "ipBlacklistSettings": {
      "en": "IP Blacklist",
      "zh": "IP 黑名单"
    },
    "loginMethod": {
      "en": "Login Method",
      "zh": "登录方式"
    },
    "loginViaDisabledCheck": {
      "en": "Disabled Password Check",
      "zh": "已禁用密码检查"
    },
    "loginViaPassword": {
      "en": "Admin Password Login",
      "zh": "Admin 密码登录"
    },
    "loginViaUserAdmin": {
      "en": "User Admin Permission",
      "zh": "用户管理员权限"
    },
    "logout": {
      "en": "Logout",
      "zh": "退出登录"
    },
    "logoutConfirmContent": {
      "en": "Are you sure you want to logout from admin panel?",
      "zh": "确定要退出管理员面板吗？"
    },
    "logoutConfirmTitle": {
      "en": "Confirm Logout",
      "zh": "确认退出"
    },
    "logoutSuccess": {
      "en": "Logout successful",
      "zh": "退出成功"
    },
    "mailWebhook": {
      "en": "Mail Webhook",
      "zh": "邮件 Webhook"
    },
    "mails": {
      "en": "Emails",
      "zh": "邮件"
    },
    "maintenance": {
      "en": "Maintenance",
      "zh": "维护"
    },
    "ok": {
      "en": "OK",
      "zh": "确定"
    },
    "qucickSetup": {
      "en": "Quick Setup",
      "zh": "快速设置"
    },
    "roleAddressConfig": {
      "en": "Role Address Config",
      "zh": "角色地址配置"
    },
    "sendBox": {
      "en": "Send Box",
      "zh": "发件箱"
    },
    "sendMail": {
      "en": "Send Mail",
      "zh": "发送邮件"
    },
    "senderAccess": {
      "en": "Sender Access Control",
      "zh": "发件权限控制"
    },
    "statistics": {
      "en": "Statistics",
      "zh": "统计"
    },
    "telegram": {
      "en": "Telegram Bot",
      "zh": "电报机器人"
    },
    "unknow": {
      "en": "Mails with unknow receiver",
      "zh": "无收件人邮件"
    },
    "user": {
      "en": "User",
      "zh": "用户"
    },
    "userOauth2Settings": {
      "en": "Oauth2 Settings",
      "zh": "Oauth2 设置"
    },
    "user_management": {
      "en": "User Management",
      "zh": "用户管理"
    },
    "user_settings": {
      "en": "User Settings",
      "zh": "用户设置"
    },
    "webhookSettings": {
      "en": "Webhook Settings",
      "zh": "Webhook 设置"
    },
    "workerconfig": {
      "en": "Worker Config",
      "zh": "Worker 配置"
    },
    "subAdmins": {
      "en": "Sub Admins",
      "zh": "子管理员"
    },
    "domainCreateCosts": {
      "en": "Domain Create Cost",
      "zh": "域名开号单价"
    },
    "roleMain": {
      "en": "Main admin",
      "zh": "主管理员"
    },
    "roleSub": {
      "en": "Sub admin",
      "zh": "子管理员"
    },
    "sessionExpiredTip": {
      "en": "Admin session expired. Enter username and password again, or go back to the login page.",
      "zh": "管理员登录已过期。请重新输入用户名和密码，或返回登录页。"
    },
    "backToLogin": {
      "en": "Back to Login",
      "zh": "返回登录"
    },
    "username": {
      "en": "Username",
      "zh": "用户名"
    },
    "password": {
      "en": "Password",
      "zh": "密码"
    },
    "loginFailed": {
      "en": "Login failed",
      "zh": "登录失败"
    }
  },
  "views.User": {
    "address_management": {
      "en": "Address Management",
      "zh": "地址管理"
    },
    "bind_address": {
      "en": "Bind Mail Address",
      "zh": "绑定邮箱地址"
    },
    "user_mail_box_tab": {
      "en": "Mail Box",
      "zh": "收件箱"
    },
    "user_settings": {
      "en": "User Settings",
      "zh": "用户设置"
    }
  },
  "views.user.UserLogin": {
    "cannotForgotPassword": {
      "en": "Mail verification is disabled or register is disabled, cannot reset password, please contact administrator",
      "zh": "未开启邮箱验证或未开启注册功能，无法重置密码，请联系管理员"
    },
    "email": {
      "en": "Email",
      "zh": "邮箱"
    },
    "forgotPassword": {
      "en": "Forgot Password",
      "zh": "忘记密码"
    },
    "login": {
      "en": "Login",
      "zh": "登录"
    },
    "loginWith": {
      "en": "Login with {provider}",
      "zh": "使用 {provider} 登录"
    },
    "loginWithPasskey": {
      "en": "Login with Passkey",
      "zh": "使用 Passkey 登录"
    },
    "password": {
      "en": "Password",
      "zh": "密码"
    },
    "pleaseCompleteTurnstile": {
      "en": "Please complete turnstile",
      "zh": "请完成人机验证"
    },
    "pleaseInput": {
      "en": "Please input email and password",
      "zh": "请输入邮箱和密码"
    },
    "pleaseInputCode": {
      "en": "Please input code",
      "zh": "请输入验证码"
    },
    "pleaseInputEmail": {
      "en": "Please input email",
      "zh": "请输入邮箱"
    },
    "pleaseLogin": {
      "en": "Please login",
      "zh": "请登录"
    },
    "register": {
      "en": "Register",
      "zh": "注册"
    },
    "resetPassword": {
      "en": "Reset Password",
      "zh": "重置密码"
    },
    "sendVerificationCode": {
      "en": "Send Verification Code",
      "zh": "发送验证码"
    },
    "verifyCode": {
      "en": "Verification Code",
      "zh": "验证码"
    },
    "verifyCodeSent": {
      "en": "Verification Code Sent, expires in {timeout} seconds",
      "zh": "验证码已发送, {timeout} 秒后失效"
    },
    "waitforVerifyCode": {
      "en": "Wait for {timeout} seconds",
      "zh": "等待{timeout}秒"
    }
  },
  "views.user.UserBar": {
    "currentUser": {
      "en": "Current Login User",
      "zh": "当前登录用户"
    },
    "fetchUserSettingsError": {
      "en": "Login password is invalid or account not exist, it may be network connection issue, please try again later.",
      "zh": "登录信息已过期或账号不存在，也可能是网络连接异常，请稍后再尝试。"
    }
  },
  "views.user.AddressManagement": {
    "actions": {
      "en": "Actions",
      "zh": "操作"
    },
    "address": {
      "en": "Address",
      "zh": "地址"
    },
    "changeMailAddress": {
      "en": "Change Address",
      "zh": "切换地址"
    },
    "create_or_bind": {
      "en": "Create or Bind",
      "zh": "创建或绑定"
    },
    "mail_count": {
      "en": "Mail Count",
      "zh": "邮件数量"
    },
    "itemCount": {
      "en": "Total",
      "zh": "总数"
    },
    "name": {
      "en": "Name",
      "zh": "名称"
    },
    "send_count": {
      "en": "Send Count",
      "zh": "发送数量"
    },
    "success": {
      "en": "success",
      "zh": "成功"
    },
    "targetUserEmail": {
      "en": "Target User Email",
      "zh": "目标用户邮箱"
    },
    "transferAddress": {
      "en": "Transfer Address",
      "zh": "转移地址"
    },
    "transferAddressTip": {
      "en": "Transfer address to another user will remove the address from your account and transfer it to another user. Are you sure to transfer the address?",
      "zh": "转移地址到其他用户将会从你的账户中移除此地址并转移给其他用户。确定要转移地址吗？"
    },
    "unbindAddress": {
      "en": "Unbind Address",
      "zh": "解绑地址"
    },
    "unbindAddressTip": {
      "en": "Before unbinding, please switch to this email address and save the email address credential.",
      "zh": "解绑前请切换到此邮箱地址并保存邮箱地址凭证。"
    }
  },
  "views.index.AccountSettings": {
    "changePassword": {
      "en": "Change Address Password",
      "zh": "修改地址密码"
    },
    "clearInbox": {
      "en": "Clear Inbox",
      "zh": "清空收件箱"
    },
    "clearInboxConfirm": {
      "en": "Are you sure to clear all emails in your inbox?",
      "zh": "确定要清空你收件箱中的所有邮件吗？"
    },
    "clearSentItems": {
      "en": "Clear Sent Items",
      "zh": "清空发件箱"
    },
    "clearSentItemsConfirm": {
      "en": "Are you sure to clear all emails in your sent items?",
      "zh": "确定要清空你发件箱中的所有邮件吗？"
    },
    "confirmPassword": {
      "en": "Confirm Password",
      "zh": "确认密码"
    },
    "deleteAccount": {
      "en": "Delete Email Address",
      "zh": "删除邮箱地址"
    },
    "deleteAccountConfirm": {
      "en": "Are you sure you want to delete this email address and all of its emails? This does not delete your user account.",
      "zh": "确定要删除当前邮箱地址及其中的所有邮件吗？此操作不会删除你的用户账号。"
    },
    "logout": {
      "en": "Log Out of Address",
      "zh": "退出地址登录"
    },
    "logoutConfirm": {
      "en": "Are you sure you want to log out of the current email address?",
      "zh": "确定要退出当前邮箱地址的登录吗？"
    },
    "newPassword": {
      "en": "New Password",
      "zh": "新密码"
    },
    "passwordChanged": {
      "en": "Password changed successfully",
      "zh": "密码修改成功"
    },
    "passwordMismatch": {
      "en": "Passwords do not match",
      "zh": "密码不匹配"
    },
    "showAddressCredential": {
      "en": "Credentials & Connection Methods",
      "zh": "地址凭证与连接方式"
    },
    "success": {
      "en": "Success",
      "zh": "成功"
    }
  },
  "views.user.UserOauth2Callback": {
    "codeNotFound": {
      "en": "code not found",
      "zh": "未找到授权码"
    },
    "logging": {
      "en": "Logging in...",
      "zh": "登录中..."
    },
    "stateNotMatch": {
      "en": "state not match",
      "zh": "state 不匹配"
    }
  },
  "views.index.Attachment": {
    "action": {
      "en": "Action",
      "zh": "操作"
    },
    "delete": {
      "en": "Delete",
      "zh": "删除"
    },
    "deleteConfirm": {
      "en": "Are you sure to delete this attachment?",
      "zh": "确定要删除此附件吗？"
    },
    "deleteSuccess": {
      "en": "Deleted successfully",
      "zh": "删除成功"
    },
    "download": {
      "en": "Download",
      "zh": "下载"
    }
  },
  "views.index.AutoReply": {
    "autoReply": {
      "en": "Auto Reply",
      "zh": "自动回复"
    },
    "enableAutoReply": {
      "en": "Enable Auto Reply",
      "zh": "启用自动回复"
    },
    "name": {
      "en": "Name",
      "zh": "名称"
    },
    "save": {
      "en": "Save",
      "zh": "保存"
    },
    "settings": {
      "en": "Settings",
      "zh": "设置"
    },
    "sourcePrefix": {
      "en": "Sender Filter",
      "zh": "发件人过滤"
    },
    "sourcePrefixPlaceholder": {
      "en": "Empty=all, prefix match, or /regex/",
      "zh": "留空=全部匹配，前缀匹配，或 /正则/"
    },
    "subject": {
      "en": "Subject",
      "zh": "主题"
    },
    "success": {
      "en": "Success",
      "zh": "成功"
    }
  },
  "views.index.SendMail": {
    "content": {
      "en": "Content",
      "zh": "内容"
    },
    "contentEmpty": {
      "en": "Content is empty",
      "zh": "内容不能为空"
    },
    "edit": {
      "en": "Edit",
      "zh": "编辑"
    },
    "fromName": {
      "en": "Your Name and Address, leave Name blank to use email address",
      "zh": "你的名称和地址，名称不填写则使用邮箱地址"
    },
    "html": {
      "en": "HTML",
      "zh": "HTML"
    },
    "options": {
      "en": "Options",
      "zh": "选项"
    },
    "preview": {
      "en": "Preview",
      "zh": "预览"
    },
    "requestAccess": {
      "en": "Request Access",
      "zh": "申请权限"
    },
    "requestAccessTip": {
      "en": "Send permission and balance are managed separately for each email address. The current address, {address}, has no available balance. Request permission for this address or contact the admin.",
      "zh": "发信权限和额度按邮箱地址独立管理。当前地址 {address} 暂无可用额度，请为此地址申请发信权限或联系管理员。"
    },
    "requestSuccess": {
      "en": "Send permission requested for the current address",
      "zh": "已为当前地址提交发信权限申请"
    },
    "rich text": {
      "en": "Rich Text",
      "zh": "富文本"
    },
    "send": {
      "en": "Send",
      "zh": "发送"
    },
    "send_balance": {
      "en": "Current Address Send Balance",
      "zh": "当前地址剩余发信额度"
    },
    "subject": {
      "en": "Subject",
      "zh": "主题"
    },
    "subjectEmpty": {
      "en": "Subject is empty",
      "zh": "主题不能为空"
    },
    "successSend": {
      "en": "Please check your sendbox. If failed, please check your balance or try again later.",
      "zh": "请查看您的发件箱, 如果失败, 请检查您的余额或稍后重试。"
    },
    "text": {
      "en": "Text",
      "zh": "文本"
    },
    "toMailEmpty": {
      "en": "Recipient address is empty",
      "zh": "收件人地址不能为空"
    },
    "toName": {
      "en": "Recipient Name and Address, leave Name blank to use email address",
      "zh": "收件人名称和地址，名称不填写则使用邮箱地址"
    },
    "tooLarge": {
      "en": "Too large file, please upload file less than 1MB.",
      "zh": "文件过大, 请上传小于1MB的文件。"
    }
  },
  "views.index.SimpleIndex": {
    "accountSettings": {
      "en": "Email Settings",
      "zh": "邮箱设置"
    },
    "addressCopied": {
      "en": "Address copied successfully",
      "zh": "地址复制成功"
    },
    "addressCredential": {
      "en": "Mail Address Credential",
      "zh": "邮箱地址凭证"
    },
    "addressCredentialTip": {
      "en": "Please copy the Mail Address Credential and you can use it to login",
      "zh": "请复制邮箱地址凭证，你可以使用它登录你的邮箱。"
    },
    "copyAddress": {
      "en": "Copy",
      "zh": "复制"
    },
    "deleteSuccess": {
      "en": "Mail deleted successfully",
      "zh": "邮件删除成功"
    },
    "exitSimpleIndex": {
      "en": "Exit Simple",
      "zh": "退出极简"
    },
    "mailCount": {
      "en": "{current} / {total} emails",
      "zh": "{current} / {total} 封邮件"
    },
    "nextPage": {
      "en": "Next",
      "zh": "下一页"
    },
    "noMails": {
      "en": "No mails found",
      "zh": "暂无邮件"
    },
    "prevPage": {
      "en": "Previous",
      "zh": "上一页"
    },
    "refreshAfter": {
      "en": "Refresh After {msg} Seconds",
      "zh": "{msg}秒后刷新"
    },
    "refreshMails": {
      "en": "Refresh",
      "zh": "刷新"
    },
    "refreshSuccess": {
      "en": "Mails refreshed successfully",
      "zh": "邮件刷新成功"
    }
  },
  "views.index.TelegramAddress": {
    "actions": {
      "en": "Actions",
      "zh": "操作"
    },
    "address": {
      "en": "Address",
      "zh": "地址"
    },
    "bind": {
      "en": "Bind",
      "zh": "绑定"
    },
    "bindAddressSuccess": {
      "en": "Bind Address Success",
      "zh": "绑定地址成功"
    },
    "changeMailAddress": {
      "en": "Change Mail Address",
      "zh": "切换邮箱地址"
    },
    "success": {
      "en": "success",
      "zh": "成功"
    },
    "unbindMailAddress": {
      "en": "Unbind Mail Address",
      "zh": "解绑邮箱地址"
    }
  },
  "views.index.AddressBar": {
    "addressCredential": {
      "en": "Mail Address Credential",
      "zh": "邮箱地址凭证"
    },
    "addressCredentialTip": {
      "en": "Please copy the Mail Address Credential and you can use it to login to your email account.",
      "zh": "请复制邮箱地址凭证，你可以使用它登录你的邮箱。"
    },
    "addressManage": {
      "en": "Manage",
      "zh": "地址管理"
    },
    "addressPassword": {
      "en": "Address Password",
      "zh": "地址密码"
    },
    "fetchAddressError": {
      "en": "Mail address credential is invalid or account not exist, it may be network connection issue, please try again later.",
      "zh": "邮箱地址凭证无效或邮箱地址不存在，也可能是网络连接异常，请稍后再尝试。"
    },
    "linkWithAddressCredential": {
      "en": "Open to auto login email link",
      "zh": "打开即可自动登录邮箱的链接"
    },
    "ok": {
      "en": "OK",
      "zh": "确定"
    },
    "userLogin": {
      "en": "User Login",
      "zh": "用户登录"
    }
  },
  "views.admin.SendBox": {
    "query": {
      "en": "Query",
      "zh": "查询"
    },
    "queryTip": {
      "en": "Please input address to query, leave blank to query all",
      "zh": "请输入地址查询, 留空则查询所有"
    }
  },
  "components.AiAdvisor": {
    "advisor": { "en": "Advisor", "zh": "顾问" },
    "aiMissing": {
      "en": "Cloudflare Workers AI is not bound on this worker, so chat is unavailable.",
      "zh": "当前 Worker 未绑定 Cloudflare Workers AI，暂时不能对话。"
    },
    "authHintAdmin": {
      "en": "Pick sub-admins, expand to choose mailboxes, or select all in a group.",
      "zh": "先选子管理员，展开后勾具体邮箱，也可以点分组全选。"
    },
    "authHintUser": {
      "en": "Choose from mailboxes saved in this browser.",
      "zh": "从本机已保存的邮箱里勾选要授权的地址。"
    },
    "authTitle": { "en": "Authorize mailboxes", "zh": "授权邮箱" },
    "authorizedCount": { "en": "{count} authorized", "zh": "已授权 {count} 个" },
    "chatCleared": { "en": "Conversation cleared", "zh": "对话已清空" },
    "chatEmpty": {
      "en": "Ask about verification codes, senders, or what a message means. Only authorized mailboxes are read.",
      "zh": "可以问验证码、是谁寄的、某封信在说什么。顾问只会读你授权过的邮箱。"
    },
    "chatFailed": { "en": "The advisor could not answer.", "zh": "顾问暂时无法回答" },
    "chatHint": {
      "en": "It only sees recent mail from authorized boxes. Pick a model on the right.",
      "zh": "顾问只能看到已授权邮箱里的最近邮件。右侧可切换模型。"
    },
    "chatPlaceholder": { "en": "Ask about these mailboxes…", "zh": "问问这些邮箱里的邮件…" },
    "chatTitle": { "en": "Advisor chat", "zh": "AI 对话" },
    "clearAll": { "en": "Clear", "zh": "清空" },
    "clearChat": { "en": "Clear chat", "zh": "清空对话" },
    "contactAdmin": {
      "en": "Ask an administrator to enable Cloudflare Workers AI.",
      "zh": "请联系管理员开放 Cloudflare Workers AI"
    },
    "customHint": {
      "en": "OpenAI-compatible or Claude-compatible HTTPS API. The key stays on this account.",
      "zh": "可填 OpenAI 或 Claude 格式的 HTTPS 接口。密钥只保存在你自己的账号下。"
    },
    "driverCf": { "en": "Cloudflare", "zh": "Cloudflare" },
    "driverCfDesc": {
      "en": "Free Workers AI models on this worker.",
      "zh": "使用本站 Workers AI 免费模型"
    },
    "driverCustom": { "en": "Custom API", "zh": "自定义" },
    "driverCustomDesc": {
      "en": "Your own OpenAI or Claude compatible API.",
      "zh": "使用你自己的 OpenAI 或 Claude 接口"
    },
    "driverHint": {
      "en": "Use one driver at a time. Cloudflare is only available if an admin enabled it for you; otherwise only custom works.",
      "zh": "Cloudflare 和自定义只能选一个。有权限才能选 Cloudflare，没有权限只能用自定义。"
    },
    "driverPick": { "en": "Which driver to use", "zh": "使用哪种驱动" },
    "driverTitle": { "en": "Model driver", "zh": "模型驱动" },
    "driverUsing": { "en": "In use", "zh": "当前使用" },
    "enableCf": { "en": "Allow Cloudflare Workers AI", "zh": "允许使用 Cloudflare Workers AI" },
    "emptyLocal": {
      "en": "No local mailboxes yet. Sign in to a mailbox first.",
      "zh": "还没有本机保存的邮箱，请先登录一只邮箱。"
    },
    "emptyMailboxes": { "en": "No mailboxes to authorize", "zh": "没有可授权的邮箱" },
    "loadFailed": { "en": "Failed to load advisor", "zh": "加载顾问失败" },
    "saveAuth": { "en": "Save authorization", "zh": "保存授权" },
    "saveFailed": { "en": "Failed to save authorization", "zh": "保存授权失败" },
    "saved": { "en": "Authorization saved", "zh": "授权已保存" },
    "searchMailbox": { "en": "Search mailboxes", "zh": "搜索邮箱" },
    "selectAll": { "en": "Select all", "zh": "全选" },
    "send": { "en": "Send", "zh": "发送" },
    "settingsTitle": { "en": "Settings", "zh": "设置" },
    "subAdmins": { "en": "Sub-admins", "zh": "子管理" },
    "subDefault": { "en": "Default (unconfigured sub-admins)", "zh": "默认（未单独配置的子管理）" },
    "subInherited": { "en": "Synced with default", "zh": "当前同步默认" },
    "subRestore": { "en": "Revert to default", "zh": "恢复默认同步" },
    "users": { "en": "Mailbox users", "zh": "用户" },
    "policyHint": {
      "en": "This only decides who may use Cloudflare. Pick your own driver in Model driver.",
      "zh": "这里只配置谁能用 Cloudflare。你自己用哪种驱动，请到「模型驱动」里二选一。"
    },
    "policyOwnDriver": {
      "en": "Your own Cloudflare / custom choice is on the Model driver tab.",
      "zh": "自己用 Cloudflare 还是自定义，请到「模型驱动」页选择。"
    },
    "policySaved": { "en": "Access policy saved", "zh": "权限已保存" },
    "allowedModels": { "en": "Allowed models", "zh": "可使用的模型" },
    "savePolicy": { "en": "Save policy", "zh": "保存权限" },
    "saveDriver": { "en": "Save and use this driver", "zh": "保存并使用此驱动" },
    "driverSaved": { "en": "Driver saved", "zh": "驱动已保存" },
    "apiFormat": { "en": "API format", "zh": "接口格式" },
    "baseUrl": { "en": "Base URL", "zh": "请求地址" },
    "modelId": { "en": "Model id", "zh": "请求模型 ID" },
    "apiKey": { "en": "API key", "zh": "API Key" },
    "apiKeyKept": { "en": "A key is already saved. Leave blank to keep it.", "zh": "已保存过密钥，留空则不改。" },
    "test": { "en": "Test", "zh": "测试" },
    "testHint": { "en": "Sends “hi” and shows status, first-token time, and full reply.", "zh": "会发送 hi，并显示状态码、首字时间和完整回复。" },
    "testStatus": { "en": "Status {status}", "zh": "状态 {status}" },
    "firstToken": { "en": "First token {ms} ms", "zh": "首字 {ms} ms" },
    "totalTime": { "en": "Complete {ms} ms", "zh": "完整 {ms} ms" },
    "modelSelect": { "en": "Model", "zh": "模型" },
    "needCustom": {
      "en": "Save a custom API model id and key first.",
      "zh": "请先填写并保存自定义模型 ID 和 Key"
    },
    "formatOpenAi": { "en": "OpenAI", "zh": "OpenAI" },
    "formatClaude": { "en": "Claude", "zh": "Claude" },
    "you": { "en": "You", "zh": "你" }
  },
  "components.ApiTokens": {
    "title": { "en": "API tokens", "zh": "API 令牌" },
    "hint": {
      "en": "Create a token for scripts. Use Authorization: Bearer <token>. Read and write are separate. Defaults to everything you can do.",
      "zh": "给脚本用的令牌。请求头 Authorization: Bearer 令牌。每个大类可勾只读 / 可写，默认全开。只能勾你这个身份能做的事。"
    },
    "usage": {
      "en": "Authorization: Bearer",
      "zh": "请求头 Authorization: Bearer"
    },
    "lastUsed": { "en": "Last used", "zh": "最近使用" },
    "neverUsed": { "en": "Never used", "zh": "从未使用" },
    "name": { "en": "Token name", "zh": "令牌名称" },
    "create": { "en": "Create token", "zh": "新建令牌" },
    "created": { "en": "Copy this token now. It will not be shown again.", "zh": "立刻复制这串令牌，关闭后不会再显示。" },
    "copy": { "en": "Copy", "zh": "复制" },
    "copied": { "en": "Copied", "zh": "已复制" },
    "revoke": { "en": "Revoke", "zh": "撤销" },
    "revokeConfirm": { "en": "Revoke this token?", "zh": "确定撤销这枚令牌？" },
    "empty": { "en": "No tokens yet.", "zh": "还没有令牌" },
    "prefix": { "en": "Prefix", "zh": "前缀" },
    "createdAt": { "en": "Created", "zh": "创建时间" },
    "read": { "en": "Read", "zh": "只读" },
    "write": { "en": "Write", "zh": "可写" },
    "details": { "en": "Details", "zh": "细分" },
    "loadFailed": { "en": "Failed to load tokens", "zh": "加载令牌失败" },
    "saveFailed": { "en": "Failed to create token", "zh": "创建令牌失败" },
    "nameRequired": { "en": "Enter a name", "zh": "请填写令牌名称" },
    "needScope": { "en": "Select at least one permission", "zh": "至少勾一项权限" },
    "downloadDoc": { "en": "Download API spec for AI", "zh": "下载给 AI 的接口说明" },
    "downloadFailed": { "en": "Failed to download the spec", "zh": "下载接口说明失败" },
    "category.address": { "en": "Mailboxes", "zh": "邮箱账号" },
    "category.mail": { "en": "Mail", "zh": "邮件" },
    "category.send": { "en": "Send mail", "zh": "发信" },
    "category.sender_access": { "en": "Sender access", "zh": "发件权限" },
    "category.settings": { "en": "Account and security settings", "zh": "账号与风控设置" },
    "category.sub_admin": { "en": "Sub-admins and quota", "zh": "子管理与额度" },
    "category.ai_advisor": { "en": "AI advisor", "zh": "AI 顾问" },
    "category.telegram": { "en": "Telegram", "zh": "Telegram" },
    "category.statistics": { "en": "Statistics", "zh": "统计" },
    "category.maintenance": { "en": "Maintenance", "zh": "系统维护" },
    "category.appearance": { "en": "Appearance", "zh": "外观" },
    "category.self_service": { "en": "Mailbox extras", "zh": "邮箱自助" },
    "scope.address.list.read": { "en": "List mailboxes", "zh": "查看邮箱列表" },
    "scope.address.credential.read": { "en": "View credentials", "zh": "查看凭证" },
    "scope.address.create.write": { "en": "Create mailbox", "zh": "创建邮箱" },
    "scope.address.password.write": { "en": "Change password", "zh": "改密码" },
    "scope.address.delete.write": { "en": "Delete mailbox", "zh": "删除邮箱" },
    "scope.address.clear_inbox.write": { "en": "Clear inbox", "zh": "清空收件箱" },
    "scope.address.clear_sent.write": { "en": "Clear sent items", "zh": "清空发件记录" },
    "scope.address.group.read": { "en": "Read groups", "zh": "查看分组" },
    "scope.address.group.write": { "en": "Manage groups", "zh": "管理分组" },
    "scope.address.note.write": { "en": "Edit notes", "zh": "改备注" },
    "scope.mail.inbox.read": { "en": "Read inbox", "zh": "查看收件 / 读信" },
    "scope.mail.inbox.write": { "en": "Delete inbox mail", "zh": "删除收件" },
    "scope.mail.unknown.read": { "en": "Read unknown mail", "zh": "查看未知邮件" },
    "scope.mail.sendbox.read": { "en": "Read sent mail", "zh": "查看发件箱" },
    "scope.mail.sendbox.write": { "en": "Delete sent mail", "zh": "删除发件记录" },
    "scope.send.mail.write": { "en": "Send mail", "zh": "发送邮件" },
    "scope.send.request.write": { "en": "Request send access", "zh": "申请发信权限" },
    "scope.sender_access.list.read": { "en": "View sender access", "zh": "查看发件权限" },
    "scope.sender_access.update.write": { "en": "Edit sender access", "zh": "改发件权限" },
    "scope.sender_access.delete.write": { "en": "Remove sender access", "zh": "删除发件权限" },
    "scope.settings.account.read": { "en": "View account settings", "zh": "查看账号设置" },
    "scope.settings.account.write": { "en": "Save account settings", "zh": "保存账号设置" },
    "scope.settings.ip.read": { "en": "View IP blacklist", "zh": "查看 IP 黑名单" },
    "scope.settings.ip.write": { "en": "Save IP blacklist", "zh": "保存 IP 黑名单" },
    "scope.settings.webhook.read": { "en": "View webhook allowlist", "zh": "查看 Webhook 白名单" },
    "scope.settings.webhook.write": { "en": "Save webhook allowlist", "zh": "保存 Webhook 白名单" },
    "scope.settings.mail_webhook.read": { "en": "View mail webhook", "zh": "查看邮件 Webhook" },
    "scope.settings.mail_webhook.write": { "en": "Save mail webhook", "zh": "保存邮件 Webhook" },
    "scope.sub_admin.list.read": { "en": "List sub-admins", "zh": "查看子管理" },
    "scope.sub_admin.ledger.read": { "en": "View quota ledger", "zh": "查看额度流水" },
    "scope.sub_admin.costs.read": { "en": "View domain costs", "zh": "查看域名单价" },
    "scope.sub_admin.manage.write": { "en": "Manage sub-admins", "zh": "管子管理账号" },
    "scope.sub_admin.quota.write": { "en": "Adjust quota", "zh": "加减额度" },
    "scope.sub_admin.costs.write": { "en": "Save domain costs", "zh": "保存域名单价" },
    "scope.ai_advisor.read": { "en": "View advisor", "zh": "查看顾问配置" },
    "scope.ai_advisor.write": { "en": "Use advisor", "zh": "使用顾问" },
    "scope.ai_advisor.policy.read": { "en": "View advisor policy", "zh": "查看顾问策略" },
    "scope.ai_advisor.policy.write": { "en": "Save advisor policy", "zh": "保存顾问策略" },
    "scope.telegram.read": { "en": "View Telegram", "zh": "查看 Telegram" },
    "scope.telegram.write": { "en": "Save Telegram", "zh": "保存 Telegram" },
    "scope.statistics.read": { "en": "View statistics", "zh": "查看统计" },
    "scope.maintenance.read": { "en": "View maintenance", "zh": "查看维护信息" },
    "scope.maintenance.write": { "en": "Run maintenance", "zh": "执行维护" },
    "scope.appearance.read": { "en": "View appearance prefs", "zh": "查看外观全局开关" },
    "scope.appearance.write": { "en": "Save appearance prefs", "zh": "保存外观全局开关" },
    "scope.self_service.read": { "en": "View mailbox extras", "zh": "查看自动回复 / Webhook / S3" },
    "scope.self_service.write": { "en": "Change mailbox extras", "zh": "改自动回复 / Webhook / S3" }
  },
  "views.admin.SendMail": {
    "content": {
      "en": "Content",
      "zh": "内容"
    },
    "contentEmpty": {
      "en": "Content is empty",
      "zh": "内容不能为空"
    },
    "edit": {
      "en": "Edit",
      "zh": "编辑"
    },
    "fromMailEmpty": {
      "en": "Sender address is empty",
      "zh": "发件人地址不能为空"
    },
    "fromName": {
      "en": "Your Name and Address, leave Name blank to use email address",
      "zh": "你的名称和地址，名称不填写则使用邮箱地址"
    },
    "html": {
      "en": "HTML",
      "zh": "HTML"
    },
    "options": {
      "en": "Options",
      "zh": "选项"
    },
    "preview": {
      "en": "Preview",
      "zh": "预览"
    },
    "rich text": {
      "en": "Rich Text",
      "zh": "富文本"
    },
    "send": {
      "en": "Send",
      "zh": "发送"
    },
    "subject": {
      "en": "Subject",
      "zh": "主题"
    },
    "subjectEmpty": {
      "en": "Subject is empty",
      "zh": "主题不能为空"
    },
    "successSend": {
      "en": "Please check your sendbox. If failed, please try again later.",
      "zh": "请查看您的发件箱, 如果失败, 请检查稍后重试。"
    },
    "text": {
      "en": "Text",
      "zh": "文本"
    },
    "toMailEmpty": {
      "en": "Recipient address is empty",
      "zh": "收件人地址不能为空"
    },
    "toName": {
      "en": "Recipient Name and Address, leave Name blank to use email address",
      "zh": "收件人名称和地址，名称不填写则使用邮箱地址"
    },
    "tooLarge": {
      "en": "Too large file, please upload file less than 1MB.",
      "zh": "文件过大, 请上传小于1MB的文件。"
    }
  },
  "views.admin.Account": {
    "actions": {
      "en": "Actions",
      "zh": "操作"
    },
    "addressCredential": {
      "en": "Mail Address Credential",
      "zh": "邮箱地址凭证"
    },
    "addressCredentialTip": {
      "en": "Please copy the Mail Address Credential and you can use it to login to your email account.",
      "zh": "请复制邮箱地址凭证，你可以使用它登录你的邮箱。"
    },
    "addressQueryTip": {
      "en": "Leave blank to query all addresses",
      "zh": "留空查询所有地址"
    },
    "clearInbox": {
      "en": "Clear Inbox",
      "zh": "清空收件箱"
    },
    "clearInboxTip": {
      "en": "Are you sure to clear inbox for this email?",
      "zh": "确定要清空这个邮箱的收件箱吗？"
    },
    "clearSentItems": {
      "en": "Clear Sent Items",
      "zh": "清空发件箱"
    },
    "clearSentItemsTip": {
      "en": "Are you sure to clear sent items for this email?",
      "zh": "确定要清空这个邮箱的发件箱吗？"
    },
    "created_at": {
      "en": "Created At",
      "zh": "创建时间"
    },
    "delete": {
      "en": "Delete",
      "zh": "删除"
    },
    "deleteAccount": {
      "en": "Delete Account",
      "zh": "删除邮箱"
    },
    "deleteTip": {
      "en": "Are you sure to delete this email?",
      "zh": "确定要删除这个邮箱吗？"
    },
    "itemCount": {
      "en": "itemCount",
      "zh": "总数"
    },
    "mail_count": {
      "en": "Mail Count",
      "zh": "邮件数量"
    },
    "multiClearInbox": {
      "en": "Multi Clear Inbox",
      "zh": "批量清空收件箱"
    },
    "multiClearInboxTip": {
      "en": "Are you sure to clear inbox for selected addresses?",
      "zh": "确定要清空选中邮箱的收件箱吗？"
    },
    "multiClearSentItems": {
      "en": "Multi Clear Sent Items",
      "zh": "批量清空发件箱"
    },
    "multiClearSentItemsTip": {
      "en": "Are you sure to clear sent items for selected addresses?",
      "zh": "确定要清空选中邮箱的发件箱吗？"
    },
    "multiDelete": {
      "en": "Multi Delete",
      "zh": "批量删除"
    },
    "multiDeleteTip": {
      "en": "Are you sure to delete selected addresses?",
      "zh": "确定要删除选中的邮箱吗？"
    },
    "name": {
      "en": "Name",
      "zh": "名称"
    },
    "newPassword": {
      "en": "New Password",
      "zh": "新密码"
    },
    "passwordResetSuccess": {
      "en": "Password reset successfully",
      "zh": "密码重置成功"
    },
    "pleaseSelectAddress": {
      "en": "Please select address",
      "zh": "请选择地址"
    },
    "query": {
      "en": "Query",
      "zh": "查询"
    },
    "resetPassword": {
      "en": "Reset Password",
      "zh": "重置密码"
    },
    "selectAll": {
      "en": "Select All of This Page",
      "zh": "全选本页"
    },
    "selectedItems": {
      "en": "Selected",
      "zh": "已选择"
    },
    "send_count": {
      "en": "Send Count",
      "zh": "发送数量"
    },
    "showCredential": {
      "en": "Show Mail Address Credential",
      "zh": "查看邮箱地址凭证"
    },
    "source_meta": {
      "en": "Source",
      "zh": "来源"
    },
    "success": {
      "en": "Success",
      "zh": "成功"
    },
    "unselectAll": {
      "en": "Unselect All",
      "zh": "取消全选"
    },
    "updated_at": {
      "en": "Update At",
      "zh": "更新时间"
    },
    "viewMails": {
      "en": "View Mails",
      "zh": "查看邮件"
    },
    "viewSendBox": {
      "en": "View SendBox",
      "zh": "查看发件箱"
    },
    "owner": {
      "en": "Created By",
      "zh": "开号人"
    },
    "ownerMain": {
      "en": "Main admin",
      "zh": "主管理员"
    },
    "ownerSub": {
      "en": "Sub admin",
      "zh": "子管理员"
    },
    "expires_at": {
      "en": "Expires At",
      "zh": "到期时间"
    },
    "multiSettings": {
      "en": "Batch Settings",
      "zh": "批量改设置"
    },
    "multiSettingsTip": {
      "en": "Reset password for selected addresses. Requests run one by one.",
      "zh": "为选中的邮箱重置密码。请求会逐个发送，不会一次打几千条。"
    },
    "editNote": {
      "en": "Note",
      "zh": "备注"
    },
    "note": {
      "en": "Note",
      "zh": "备注"
    },
    "notePlaceholder": {
      "en": "Optional. Shown in the name column for you only.",
      "zh": "可选。只有你自己能看见，有备注时名称栏显示备注。"
    },
    "saveNote": {
      "en": "Save note",
      "zh": "保存备注"
    },
    "allGroups": {
      "en": "All groups",
      "zh": "全部分组"
    }
  },
  "views.admin.AddressGroups": {
    "actions": { "en": "Actions", "zh": "操作" },
    "addMember": { "en": "Add mailboxes", "zh": "加入邮箱" },
    "create": { "en": "Create group", "zh": "新建分组" },
    "created_at": { "en": "Created At", "zh": "创建时间" },
    "delete": { "en": "Delete", "zh": "删除" },
    "deleteConfirm": { "en": "Delete group {name}?", "zh": "确定删除分组 {name}？" },
    "emptyMembers": { "en": "No mailboxes in this group", "zh": "这个分组还没有邮箱" },
    "limitsTip": {
      "en": "Main admin has no group cap. These numbers only limit sub-admins and mailbox users.",
      "zh": "主管理分组不限个数。这里只改子管理和邮箱用户的上限。"
    },
    "memberCount": { "en": "Members", "zh": "邮箱数" },
    "members": { "en": "Members", "zh": "成员" },
    "name": { "en": "Name", "zh": "名称" },
    "namePlaceholder": { "en": "Group name", "zh": "分组名称" },
    "nameRequired": { "en": "Enter a group name", "zh": "请填写分组名称" },
    "query": { "en": "Search", "zh": "搜索" },
    "remove": { "en": "Remove", "zh": "移出" },
    "rename": { "en": "Rename", "zh": "改名" },
    "save": { "en": "Save", "zh": "保存" },
    "saveLimits": { "en": "Save limits", "zh": "保存上限" },
    "searchAddress": { "en": "Search mailbox", "zh": "搜索邮箱" },
    "subLimit": { "en": "Sub-admin group limit", "zh": "子管理分组上限" },
    "success": { "en": "Success", "zh": "成功" },
    "tip": {
      "en": "Groups and notes are private to the current login.",
      "zh": "分组和备注只属于当前登录身份，别人看不见。"
    },
    "unlimitedUsed": { "en": "Used {used}, no cap", "zh": "已用 {used} 个，不限上限" },
    "usedOfMax": { "en": "Used {used} / {max}", "zh": "已用 {used} / {max}" },
    "userLimit": { "en": "Mailbox user group limit", "zh": "邮箱用户分组上限" }
  },
  "views.admin.UserOauth2Settings": {
    "addOauth2": {
      "en": "Add Oauth2",
      "zh": "添加 Oauth2"
    },
    "delete": {
      "en": "Delete",
      "zh": "删除"
    },
    "enable": {
      "en": "Enable",
      "zh": "启用"
    },
    "enableEmailFormat": {
      "en": "Enable Email Format",
      "zh": "启用邮箱格式转换"
    },
    "enableMailAllowList": {
      "en": "Enable Mail Address Allow List(Manually enterable)",
      "zh": "启用邮件地址白名单(可手动输入, 回车增加)"
    },
    "icon": {
      "en": "Icon (SVG, please ensure trusted source)",
      "zh": "图标 (SVG, 请确保来源可信)"
    },
    "iconPreview": {
      "en": "Preview",
      "zh": "预览"
    },
    "mailAllowList": {
      "en": "Mail Address Allow List",
      "zh": "邮件地址白名单"
    },
    "manualInputPrompt": {
      "en": "Type and press Enter to add",
      "zh": "输入后按回车键添加"
    },
    "name": {
      "en": "Name",
      "zh": "名称"
    },
    "oauth2Type": {
      "en": "Oauth2 Type",
      "zh": "Oauth2 类型"
    },
    "save": {
      "en": "Save",
      "zh": "保存"
    },
    "successTip": {
      "en": "Save Success",
      "zh": "保存成功"
    },
    "tip": {
      "en": "Third-party login will automatically use the user's email to register an account (the same email will be regarded as the same account), this account is the same as the registered account, and you can also set the password through the forget password",
      "zh": "第三方登录会自动使用用户邮箱注册账号(邮箱相同将视为同一账号), 此账号和注册的账号相同, 也可以通过忘记密码设置密码"
    },
    "userEmailFormat": {
      "en": "Email Regex Pattern",
      "zh": "邮箱正则表达式"
    },
    "userEmailFormatTip": {
      "en": "Use regex to transform email. Example: ^(.+)@old\\.com$ with $1@new.com",
      "zh": "使用正则转换邮箱。示例: ^(.+)@old\\.com$ 配合 $1@new.com"
    },
    "userEmailReplace": {
      "en": "Replace Template",
      "zh": "替换模板"
    }
  },
  "views.admin.Statistics": {
    "activeAddressCount30days": {
      "en": "30 days Active Address Count",
      "zh": "30天活跃邮箱地址总数"
    },
    "activeAddressCount7days": {
      "en": "7 days Active Address Count",
      "zh": "7天活跃邮箱地址总数"
    },
    "addressCount": {
      "en": "Address Count",
      "zh": "邮箱地址总数"
    },
    "mailCount": {
      "en": "Mail Count",
      "zh": "邮件总数"
    },
    "sendMailCount": {
      "en": "Send Mail Count",
      "zh": "发送邮件总数"
    },
    "userCount": {
      "en": "User Count",
      "zh": "用户总数"
    }
  },
  "views.admin.SenderAccess": {
    "action": {
      "en": "Action",
      "zh": "操作"
    },
    "address": {
      "en": "Address",
      "zh": "地址"
    },
    "balance": {
      "en": "Balance",
      "zh": "余额"
    },
    "created_at": {
      "en": "Created At",
      "zh": "创建时间"
    },
    "delete": {
      "en": "Delete",
      "zh": "删除"
    },
    "deleteTip": {
      "en": "Are you sure to delete this?",
      "zh": "确定删除吗？"
    },
    "disable": {
      "en": "Disable",
      "zh": "禁用"
    },
    "enable": {
      "en": "Enable",
      "zh": "启用"
    },
    "is_enabled": {
      "en": "Is Enabled",
      "zh": "是否启用"
    },
    "itemCount": {
      "en": "itemCount",
      "zh": "总数"
    },
    "modalTip": {
      "en": "Please input the sender balance",
      "zh": "请输入发件额度"
    },
    "modify": {
      "en": "Modify",
      "zh": "修改"
    },
    "ok": {
      "en": "OK",
      "zh": "确定"
    },
    "query": {
      "en": "Query",
      "zh": "查询"
    },
    "success": {
      "en": "Success",
      "zh": "成功"
    }
  },
  "views.index.LocalAddress": {
    "actions": {
      "en": "Actions",
      "zh": "操作"
    },
    "address": {
      "en": "Address",
      "zh": "地址"
    },
    "bindAddressSuccess": {
      "en": "Bind Address Success",
      "zh": "绑定地址成功"
    },
    "changeMailAddress": {
      "en": "Change Mail Address",
      "zh": "切换邮箱地址"
    },
    "create_or_bind": {
      "en": "Add Mailbox",
      "zh": "添加邮箱"
    },
    "success": {
      "en": "success",
      "zh": "成功"
    },
    "tip": {
      "en": "These addresses are stored in your browser, maybe loss if you clear the browser cache.",
      "zh": "这些地址存储在您的浏览器中，如果您清除浏览器缓存，可能会丢失。"
    },
    "unbindMailAddress": {
      "en": "Remove from this device",
      "zh": "从本机列表移除"
    },
    "multiDelete": {
      "en": "Remove Selected",
      "zh": "批量移除"
    },
    "multiDeleteTip": {
      "en": "Remove selected mailboxes from this browser only. This does not delete them on the server.",
      "zh": "仅从本机列表移除选中邮箱，不会在服务器上删除别人的号。"
    },
    "multiSettings": {
      "en": "Batch Settings",
      "zh": "批量改设置"
    },
    "multiSettingsTip": {
      "en": "Change password for selected local mailboxes. Requests run one by one with each mailbox credential.",
      "zh": "为本地选中的邮箱逐个修改密码，请求按各自凭据循环调用。"
    },
    "selectAll": {
      "en": "Select All",
      "zh": "全选"
    },
    "unselectAll": {
      "en": "Unselect All",
      "zh": "取消全选"
    },
    "selectedItems": {
      "en": "Selected",
      "zh": "已选择"
    },
    "pleaseSelectAddress": {
      "en": "Please select address",
      "zh": "请选择地址"
    },
    "newPassword": {
      "en": "New Password",
      "zh": "新密码"
    },
    "newPasswordRequired": {
      "en": "Please enter a new password",
      "zh": "请输入新密码"
    },
    "allGroups": {
      "en": "All groups",
      "zh": "全部分组"
    },
    "editNote": {
      "en": "Note",
      "zh": "备注"
    }
  },
  "views.admin.DatabaseManager": {
    "code_db_version": {
      "en": "Code Needed DB Version",
      "zh": "需要的数据库版本"
    },
    "current_db_version": {
      "en": "Current DB Version",
      "zh": "当前数据库版本"
    },
    "init": {
      "en": "Initialize Database",
      "zh": "初始化数据库"
    },
    "initializationSuccess": {
      "en": "Database initialized successfully",
      "zh": "数据库初始化成功"
    },
    "migration": {
      "en": "Migrate Database",
      "zh": "升级数据库 Schema"
    },
    "migrationSuccess": {
      "en": "Database migrated successfully",
      "zh": "数据库升级成功"
    },
    "need_initialization_tip": {
      "en": "Database initialization is required. Please initialize the database.",
      "zh": "需要初始化数据库，请初始化数据库"
    },
    "need_migration_tip": {
      "en": "Database migration is required. Please migrate the database.",
      "zh": "需要迁移数据库，请迁移数据库"
    }
  },
  "views.admin.IpBlacklistSettings": {
    "asn_blacklist": {
      "en": "ASN Organization Blacklist",
      "zh": "ASN 组织（运营商）黑名单"
    },
    "asn_blacklist_placeholder": {
      "en": "Enter ASN organization (e.g., Google, Amazon)",
      "zh": "输入 ASN 组织名称（例如：Google, Amazon）"
    },
    "daily_request_limit": {
      "en": "Daily Request Limit",
      "zh": "每日请求次数上限"
    },
    "daily_request_limit_placeholder": {
      "en": "Enter limit (e.g., 1000)",
      "zh": "输入限制次数（例如：1000）"
    },
    "enable_daily_limit": {
      "en": "Enable Daily Request Limit",
      "zh": "启用每日请求限流"
    },
    "enable_daily_limit_tip": {
      "en": "Limit the number of API requests per IP address per day",
      "zh": "限制每个 IP 地址每天的 API 请求次数"
    },
    "enable_ip_blacklist": {
      "en": "Enable IP Blacklist",
      "zh": "启用 IP 黑名单"
    },
    "enable_ip_whitelist": {
      "en": "Enable IP Whitelist (Strict)",
      "zh": "启用 IP 白名单（严格模式）"
    },
    "enable_tip": {
      "en": "Block IPs matching blacklist patterns from accessing rate-limited APIs",
      "zh": "阻止匹配黑名单的 IP 访问限流 API"
    },
    "enable_whitelist_tip": {
      "en": "Strict mode: ONLY IPs matching the whitelist can access rate-limited APIs. All other IPs will be denied.",
      "zh": "严格模式：仅允许匹配白名单的 IP 访问限流 API，其他所有 IP 将被拒绝"
    },
    "fingerprint_blacklist": {
      "en": "Browser Fingerprint Blacklist",
      "zh": "浏览器指纹黑名单"
    },
    "fingerprint_blacklist_placeholder": {
      "en": "Enter fingerprint ID (e.g., a1b2c3d4e5f6g7h8)",
      "zh": "输入指纹 ID（例如：a1b2c3d4e5f6g7h8）"
    },
    "ip_blacklist": {
      "en": "IP Blacklist Patterns",
      "zh": "IP 黑名单匹配模式"
    },
    "ip_blacklist_placeholder": {
      "en": "Enter pattern (e.g., 192.168.1 or ^10\\.0\\.0\\.5$)",
      "zh": "输入匹配模式（例如：192.168.1 或 ^10\\.0\\.0\\.5$）"
    },
    "ip_whitelist": {
      "en": "IP Whitelist Patterns",
      "zh": "IP 白名单匹配模式"
    },
    "ip_whitelist_placeholder": {
      "en": "Exact IP (e.g., 1.2.3.4) or anchored regex (e.g., ^192\\.168\\.1\\.\\d+$)",
      "zh": "精确 IP(如 1.2.3.4)或锚定正则(如 ^192\\.168\\.1\\.\\d+$)"
    },
    "manualInputPrompt": {
      "en": "Type pattern and press Enter to add",
      "zh": "输入匹配模式后按回车键添加"
    },
    "save": {
      "en": "Save",
      "zh": "保存"
    },
    "successTip": {
      "en": "Save Success",
      "zh": "保存成功"
    },
    "tip_asn": {
      "en": "ASN Organization: Block by ISP/provider. Case-insensitive text matching or regex.",
      "zh": "ASN 组织：根据运营商/ISP 拉黑。支持不区分大小写的文本匹配或正则表达式。"
    },
    "tip_daily_limit": {
      "en": "Daily Limit: Restrict the maximum number of requests per IP address per day (1-1000000).",
      "zh": "每日限流：限制单个 IP 地址每天最多请求次数（1-1000000）。"
    },
    "tip_fingerprint": {
      "en": "Browser Fingerprint: Block by browser fingerprint. Supports exact matching or regex patterns.",
      "zh": "浏览器指纹：根据浏览器指纹拉黑。支持完全匹配或正则表达式。"
    },
    "tip_ip": {
      "en": "IP Blacklist: Supports text matching (e.g., \"192.168.1\") or regex (e.g., \"^10\\.0\\.0\\.5$\").",
      "zh": "IP 黑名单：支持文本匹配（如 \"192.168.1\"）或正则表达式（如 \"^10\\.0\\.0\\.5$\"）。"
    },
    "tip_scope": {
      "en": "Applies to: Create Address, Send Mail, External Send Mail API, User Registration, Verify Code",
      "zh": "作用范围：创建邮箱地址、发送邮件、外部发送邮件 API、用户注册、验证码验证"
    },
    "tip_whitelist": {
      "en": "IP Whitelist: Strict allowlist — plain entries must be EXACT IP matches (no substring). Use anchored regex (^...$) for ranges. Whitelisted IPs skip blacklist checks.",
      "zh": "IP 白名单: 严格放行名单——纯文本必须是精确 IP(不支持子串匹配), 批量放行请用锚定正则 ^...$. 命中白名单的 IP 将跳过黑名单检查."
    },
    "title": {
      "en": "IP Blacklist Settings",
      "zh": "IP 黑名单设置"
    },
    "whitelist_empty_warning": {
      "en": "IP whitelist is enabled but the list is empty. This is ignored by the server to prevent lockout. Please add at least one entry before enabling.",
      "zh": "IP 白名单已启用但列表为空，服务端将忽略该开关以防止锁死。请先添加至少一条白名单条目再启用。"
    }
  },
  "views.admin.UserAddressManagement": {
    "mail_count": {
      "en": "Mail Count",
      "zh": "邮件数量"
    },
    "name": {
      "en": "Name",
      "zh": "名称"
    },
    "send_count": {
      "en": "Send Count",
      "zh": "发送数量"
    },
    "success": {
      "en": "success",
      "zh": "成功"
    }
  },
  "views.admin.UserManagement": {
    "actions": {
      "en": "Actions",
      "zh": "操作"
    },
    "address_count": {
      "en": "Address Count",
      "zh": "地址数量"
    },
    "changeRole": {
      "en": "Change Role",
      "zh": "更改角色"
    },
    "createUser": {
      "en": "Create User",
      "zh": "创建用户"
    },
    "created_at": {
      "en": "Created At",
      "zh": "创建时间"
    },
    "delete": {
      "en": "Delete",
      "zh": "删除"
    },
    "deleteUser": {
      "en": "Delete User",
      "zh": "删除用户"
    },
    "deleteUserTip": {
      "en": "Are you sure you want to delete this user?",
      "zh": "确定要删除此用户吗？"
    },
    "domains": {
      "en": "Domains",
      "zh": "域名"
    },
    "email": {
      "en": "Email",
      "zh": "邮箱"
    },
    "itemCount": {
      "en": "itemCount",
      "zh": "总数"
    },
    "password": {
      "en": "Password",
      "zh": "密码"
    },
    "pleaseInput": {
      "en": "Please input complete information",
      "zh": "请输入完整信息"
    },
    "prefix": {
      "en": "Prefix",
      "zh": "前缀"
    },
    "query": {
      "en": "Query",
      "zh": "查询"
    },
    "resetPassword": {
      "en": "Reset Password",
      "zh": "重置密码"
    },
    "role": {
      "en": "Role",
      "zh": "角色"
    },
    "roleDonotExist": {
      "en": "Current Role does not exist",
      "zh": "当前角色不存在"
    },
    "success": {
      "en": "Success",
      "zh": "成功"
    },
    "userAddressManagement": {
      "en": "Address Management",
      "zh": "地址管理"
    },
    "user_email": {
      "en": "User Email",
      "zh": "用户邮箱"
    }
  },
  "views.admin.Telegram": {
    "enable": {
      "en": "Enable",
      "zh": "启用"
    },
    "enableGlobalMailPush": {
      "en": "Enable Global Mail Push(Manually input telegram Chat ID)",
      "zh": "启用全局邮件推送(手动输入邮箱管理员的 telegram Chat ID, 回车增加)"
    },
    "enableTelegramAllowList": {
      "en": "Enable Telegram Allow List(Manually input Chat ID)",
      "zh": "启用 Telegram 白名单(手动输入 Chat ID, 回车增加)"
    },
    "globalMailPushList": {
      "en": "Global Mail Push Chat ID List",
      "zh": "全局邮件推送 Chat ID 列表"
    },
    "globalMailPushListTip": {
      "en": "Support chat_id of private chat/group/channel. You can send a message to your bot, then visit this link to see chat_id, https://api.telegram.org/bot<Replace with your BOT TOKEN>/getUpdates",
      "zh": "支持对话/群组/频道的 Chat ID, 您可以发送一条消息给您的机器人，然后访问此链接来查看 chat_id, https://api.telegram.org/bot<这里替换成您的 BOT TOKEN>/getUpdates"
    },
    "init": {
      "en": "Init",
      "zh": "初始化"
    },
    "manualInputPrompt": {
      "en": "Type and press Enter to add",
      "zh": "输入后按回车键添加"
    },
    "miniAppUrl": {
      "en": "Telegram Mini App URL",
      "zh": "电报小程序 URL(请输入你部署的电报小程序网页地址)"
    },
    "save": {
      "en": "Save",
      "zh": "保存"
    },
    "status": {
      "en": "Check Status",
      "zh": "查看状态"
    },
    "successTip": {
      "en": "Success",
      "zh": "成功"
    },
    "telegramAllowList": {
      "en": "Telegram Allow List(Manually input telegram Chat ID)",
      "zh": "Telegram 白名单(手动输入 Chat ID, 回车增加)"
    }
  },
  "views.admin.CreateAccount": {
    "address": {
      "en": "Address",
      "zh": "地址"
    },
    "addressCredential": {
      "en": "Mail Address Credential",
      "zh": "邮箱地址凭证"
    },
    "addressCredentialTip": {
      "en": "Please copy the Mail Address Credential and you can use it to login to your email account.",
      "zh": "请复制邮箱地址凭证，你可以使用它登录你的邮箱。"
    },
    "addressPassword": {
      "en": "Address Password",
      "zh": "地址密码"
    },
    "creatNewEmail": {
      "en": "Create New Email",
      "zh": "创建新邮箱"
    },
    "enableCustomSubdomain": {
      "en": "Use Custom Subdomain",
      "zh": "使用自定义子域名"
    },
    "enablePrefix": {
      "en": "If enable Prefix",
      "zh": "是否启用前缀"
    },
    "enableRandomSubdomain": {
      "en": "Use Random Subdomain",
      "zh": "启用随机子域名"
    },
    "fillInAllFields": {
      "en": "Please fill in all fields",
      "zh": "请填写完整信息"
    },
    "linkWithAddressCredential": {
      "en": "Open to auto login email link",
      "zh": "打开即可自动登录邮箱的链接"
    },
    "normalSubdomain": {
      "en": "Normal Domain",
      "zh": "普通域名"
    },
    "randomSubdomainTip": {
      "en": "When enabled, the created address will use a random subdomain. Recommended for receiving only. Requires a wildcard MX DNS record on the base domain — see the random subdomain docs.",
      "zh": "启用后，创建出来的地址会自动挂在随机子域名下，建议仅用于收件。需要在基础域名 DNS 中配置通配 MX 记录，详见随机子域名文档。"
    },
    "successTip": {
      "en": "Success Created",
      "zh": "创建成功"
    },
    "expireDays": {
      "en": "Expire in days",
      "zh": "到期天数"
    },
    "expireDaysSubTip": {
      "en": "Sub admins can set at most 90 days.",
      "zh": "子管理员最长可设置 90 天。"
    },
    "note": {
      "en": "Note",
      "zh": "备注"
    },
    "notePlaceholder": {
      "en": "Optional private note",
      "zh": "可选，仅自己可见的备注"
    },
    "group": {
      "en": "Group",
      "zh": "分组"
    },
    "groupAuto": {
      "en": "Auto by creator",
      "zh": "按开号人自动分组"
    }
  },
  "views.admin.UserSettings": {
    "emailCheckRegex": {
      "en": "Email Check Regex (e.g. ^[^.]+{'@'}.+$ to disallow dots before {'@'})",
      "zh": "邮箱正则校验 (例如 ^[^.]+{'@'}.+$ 禁止{'@'}前面有.)"
    },
    "enable": {
      "en": "Enable",
      "zh": "启用"
    },
    "enableEmailCheckRegex": {
      "en": "Enable Email Check Regex",
      "zh": "启用邮箱正则校验"
    },
    "enableMailAllowList": {
      "en": "Enable Mail Address Allow List(Manually enterable)",
      "zh": "启用邮件地址白名单(可手动输入, 回车增加)"
    },
    "enableMailVerify": {
      "en": "Enable Mail Verify (Send address must be an address in the system with a balance and can send mail normally)",
      "zh": "启用邮件验证(发送地址必须是系统中能有余额且能正常发送邮件的地址)"
    },
    "enableUserRegister": {
      "en": "Allow User Register",
      "zh": "允许用户注册"
    },
    "mailAllowList": {
      "en": "Mail Address Allow List",
      "zh": "邮件地址白名单"
    },
    "manualInputPrompt": {
      "en": "Type and press Enter to add",
      "zh": "输入后按回车键添加"
    },
    "maxAddressCount": {
      "en": "Maximum number of email addresses that can be binded (0 = Unlimited)",
      "zh": "可绑定最大邮箱地址数量（0 为不限制）"
    },
    "save": {
      "en": "Save",
      "zh": "保存"
    },
    "successTip": {
      "en": "Save Success",
      "zh": "保存成功"
    },
    "verifyMailSender": {
      "en": "Verify Mail Sender",
      "zh": "验证邮件发送地址"
    }
  },
  "views.admin.RoleAddressConfig": {
    "maxAddressCount": {
      "en": "Max Address Count (0 = Unlimited)",
      "zh": "最大地址数量（0 为不限制）"
    },
    "noRolesAvailable": {
      "en": "No roles available in system config",
      "zh": "系统配置中没有可用的角色"
    },
    "notConfigured": {
      "en": "Not Configured (Use Global Settings)",
      "zh": "未配置（使用全局设置）"
    },
    "role": {
      "en": "Role",
      "zh": "角色"
    },
    "roleConfigDesc": {
      "en": "Configure maximum address count for each user role. Role-based limits take priority over global settings. Set 0 for unlimited.",
      "zh": "为每个用户角色配置最大地址数量。角色配置优先于全局设置。设置为 0 表示不限制。"
    },
    "save": {
      "en": "Save",
      "zh": "保存"
    },
    "successTip": {
      "en": "Success",
      "zh": "成功"
    }
  },
  "views.common.Appearance": {
    "autoLoadRemoteImages": {
      "en": "Automatically load external images in mail body",
      "zh": "自动加载邮件正文中的外部图片"
    },
    "autoRefreshInterval": {
      "en": "Auto Refresh Interval(Sec)",
      "zh": "自动刷新间隔(秒)"
    },
    "bottom": {
      "en": "bottom",
      "zh": "底部"
    },
    "globalTabplacement": {
      "en": "Global Tab Placement",
      "zh": "全局选项卡位置"
    },
    "left": {
      "en": "left",
      "zh": "左侧"
    },
    "mailboxSplitSize": {
      "en": "Left list width in two-column mailbox view",
      "zh": "邮箱双栏视图左侧列表宽度占比"
    },
    "mailListView": {
      "en": "Full-width mailbox list view",
      "zh": "邮箱全宽列表视图"
    },
    "mailListPreviewLineClamp": {
      "en": "Body Preview Lines",
      "zh": "正文预览行数"
    },
    "off": {
      "en": "Off",
      "zh": "关闭"
    },
    "preferShowTextMail": {
      "en": "Display text Mail by default",
      "zh": "默认以文本显示邮件"
    },
    "right": {
      "en": "right",
      "zh": "右侧"
    },
    "top": {
      "en": "top",
      "zh": "顶部"
    },
    "useIframeShowMail": {
      "en": "Use iframe Show HTML Mail",
      "zh": "使用iframe显示HTML邮件"
    },
    "useSideMargin": {
      "en": "Turn on the side margins on the left and right sides of the page",
      "zh": "开启页面左右两侧侧边距"
    },
    "useSimpleIndex": {
      "en": "Use Simple Index",
      "zh": "使用极简主页"
    },
    "useUTCDate": {
      "en": "Use UTC Date",
      "zh": "使用 UTC 时间"
    },
    "hideAboutTitle": {
      "en": "Hide About and GitHub links",
      "zh": "隐藏「关于」与跳转入口"
    },
    "hideAboutDesc": {
      "en": "Controls the About tab, Header version/GitHub button, and the GitHub link. Hidden items are removed from the DOM so neighboring tabs shift in.",
      "zh": "控制「关于」页、页头版本号/GitHub 按钮，以及 GitHub 跳转。隐藏后从页面移除，旁边的选项会自动靠拢。"
    },
    "hideAboutMain": {
      "en": "Hide for main admin",
      "zh": "对主管理员隐藏"
    },
    "hideAboutSub": {
      "en": "Hide for sub-admin",
      "zh": "对子管理员隐藏"
    },
    "hideAboutUser": {
      "en": "Hide for mailbox users",
      "zh": "对个人用户（邮箱登录）隐藏"
    },
    "successTip": {
      "en": "Success",
      "zh": "成功"
    }
  },
  "views.admin.AccountSettings": {
    "actions": {
      "en": "Actions",
      "zh": "操作"
    },
    "add": {
      "en": "Add",
      "zh": "添加"
    },
    "address_block_list": {
      "en": "Address Block Keywords for Users(Admin can skip)",
      "zh": "邮件地址屏蔽关键词(管理员可跳过检查)"
    },
    "address_block_list_placeholder": {
      "en": "Please enter the keywords you want to block",
      "zh": "请输入您想要屏蔽的关键词"
    },
    "block_receive_unknow_address_email": {
      "en": "Block receive unknow address email",
      "zh": "禁止接收未知地址邮件"
    },
    "cancel": {
      "en": "Cancel",
      "zh": "取消"
    },
    "config": {
      "en": "Config",
      "zh": "配置"
    },
    "create_address_subdomain_match": {
      "en": "Allow Subdomain Suffix Match When Creating Address",
      "zh": "创建邮箱时允许子域名后缀匹配"
    },
    "create_address_subdomain_match_env_locked": {
      "en": "Worker env ENABLE_CREATE_ADDRESS_SUBDOMAIN_MATCH is currently false. The saved admin switch can be modified, but it will not take effect until env is enabled or removed.",
      "zh": "当前 Worker 环境变量 ENABLE_CREATE_ADDRESS_SUBDOMAIN_MATCH 为 false。后台开关仍可保存，但在 env 打开或移除前不会生效。"
    },
    "create_address_subdomain_match_follow_env": {
      "en": "Follow Environment Variable",
      "zh": "跟随环境变量"
    },
    "create_address_subdomain_match_follow_env_note": {
      "en": "Choosing \"Follow Environment Variable\" clears the admin override and returns to the unset state. The effective result is still controlled by the Worker env and the precedence rules.",
      "zh": "选择“跟随环境变量”会清空后台覆盖，恢复为未设置状态；最终是否开启仍由 Worker env 和优先级规则决定。"
    },
    "create_address_subdomain_match_force_disable": {
      "en": "Force Disable",
      "zh": "强制关闭"
    },
    "create_address_subdomain_match_force_enable": {
      "en": "Force Enable",
      "zh": "强制开启"
    },
    "create_address_subdomain_match_note": {
      "en": "RANDOM_SUBDOMAIN_DOMAINS already allows random or manual subdomains for listed base domains. This switch additionally allows API callers to specify subdomains under other allowed base domains.",
      "zh": "RANDOM_SUBDOMAIN_DOMAINS 已允许在所列基础域名下随机生成或手动输入子域名；此开关额外允许 API 在其他已授权基础域名下指定子域名。"
    },
    "create_address_subdomain_match_tip": {
      "en": "Only affects /api/new_address and /admin/new_address domain validation. Example: when enabled, foo.example.com can match configured base domain example.com.",
      "zh": "仅影响 /api/new_address 和 /admin/new_address 的域名校验。例如开启后，foo.example.com 可以匹配已配置的基础域名 example.com。"
    },
    "delete_rule": {
      "en": "Delete",
      "zh": "删除"
    },
    "delete_rule_confirm": {
      "en": "Are you sure you want to delete this rule?",
      "zh": "确定要删除这条规则吗？"
    },
    "delete_success": {
      "en": "Delete Success",
      "zh": "删除成功"
    },
    "domain_list": {
      "en": "Domain List (Optional)",
      "zh": "域名列表（可选）"
    },
    "email_forwarding_config": {
      "en": "Email Forwarding Configuration",
      "zh": "邮件转发配置"
    },
    "forward_address": {
      "en": "Forward Address",
      "zh": "转发地址"
    },
    "forward_address_required": {
      "en": "Forward address is required",
      "zh": "转发地址不能为空"
    },
    "forward_placeholder": {
      "en": "forward@example.com",
      "zh": "forward@example.com"
    },
    "forwarding_rule_warning": {
      "en": "Each rule will run independently. Forward address needs to be a verified address.",
      "zh": "每条规则独立运行，转发地址需要为已验证的地址。"
    },
    "fromBlockList": {
      "en": "Block Keywords for receive email",
      "zh": "接收邮件地址屏蔽关键词"
    },
    "manualInputPrompt": {
      "en": "Type and press Enter to add",
      "zh": "输入后按回车键添加"
    },
    "match_all": {
      "en": "All",
      "zh": "全部"
    },
    "match_any": {
      "en": "Any",
      "zh": "任一"
    },
    "noLimitSendAddressList": {
      "en": "No Balance Limit Send Address List",
      "zh": "无余额限制发送地址列表"
    },
    "regex_invalid": {
      "en": "Invalid regex pattern",
      "zh": "无效的正则表达式"
    },
    "regex_too_long": {
      "en": "Regex pattern too long (max 200 characters)",
      "zh": "正则表达式过长（最大200字符）"
    },
    "rule_index": {
      "en": "Rule",
      "zh": "规则"
    },
    "save": {
      "en": "Save",
      "zh": "保存"
    },
    "select_domain": {
      "en": "Select Domain",
      "zh": "选择域名"
    },
    "send_address_block_list": {
      "en": "Address Block Keywords for send email",
      "zh": "发送邮件地址屏蔽关键词"
    },
    "send_mail_daily_limit": {
      "en": "Daily Limit",
      "zh": "每日额度"
    },
    "send_mail_daily_limit_invalid": {
      "en": "Daily limit must be an integer greater than or equal to -1",
      "zh": "每日额度必须是大于等于 -1 的整数"
    },
    "send_mail_limit": {
      "en": "Send Mail Limit",
      "zh": "发信额度"
    },
    "send_mail_limit_tip": {
      "en": "This applies to all send channels. Use -1 for unlimited and 0 to block sending.",
      "zh": "对全部发信渠道生效。-1 表示无限，0 表示禁止发送。"
    },
    "send_mail_monthly_limit": {
      "en": "Monthly Limit",
      "zh": "每月额度"
    },
    "send_mail_monthly_limit_invalid": {
      "en": "Monthly limit must be an integer greater than or equal to -1",
      "zh": "每月额度必须是大于等于 -1 的整数"
    },
    "source_match_mode": {
      "en": "Match Mode",
      "zh": "匹配模式"
    },
    "source_patterns": {
      "en": "Source Address Regex (Optional)",
      "zh": "来源地址正则（可选）"
    },
    "source_patterns_placeholder": {
      "en": "e.g. gmail.com",
      "zh": "例如: gmail.com"
    },
    "source_patterns_tip": {
      "en": "Domain list filters by recipient address, source regex filters by sender address. Both conditions must match for forwarding (AND logic). Leave either empty to skip that filter.",
      "zh": "域名列表按收件地址过滤，来源正则按发件地址过滤，两者均为可选。同时配置时需同时满足（AND 逻辑），留空则跳过该条件。"
    },
    "successTip": {
      "en": "Save Success",
      "zh": "保存成功"
    },
    "tip": {
      "en": "You can manually input the following multiple select input and enter",
      "zh": "您可以手动输入以下多选输入框, 回车增加"
    },
    "verified_address_list": {
      "en": "Verified Address List(Can send email by cf internal api)",
      "zh": "已验证地址列表(可通过 cf 内部 api 发送邮件)"
    }
  },
  "views.admin.Mails": {
    "addressQueryTip": {
      "en": "Leave blank to query all addresses",
      "zh": "留空查询所有地址"
    },
    "query": {
      "en": "Query",
      "zh": "查询"
    }
  },
  "views.admin.Maintenance": {
    "addCustomSql": {
      "en": "Add Custom SQL",
      "zh": "添加自定义 SQL"
    },
    "addressCreateLabel": {
      "en": "Cleanup the address created before n days",
      "zh": "清理 n 天前创建的地址"
    },
    "autoCleanup": {
      "en": "Auto cleanup",
      "zh": "自动清理"
    },
    "basicCleanup": {
      "en": "Basic Cleanup",
      "zh": "基础清理"
    },
    "cleanupNow": {
      "en": "Cleanup now",
      "zh": "立即清理"
    },
    "cleanupSuccess": {
      "en": "Cleanup success",
      "zh": "清理成功"
    },
    "cronTip": {
      "en": "Enable cron cleanup, need to configure [crons] in worker, please refer to the document, setting 0 days means clear all",
      "zh": "启用定时清理, 需在 worker 配置 [crons] 参数, 请参考文档, 配置为 0 天表示全部清空"
    },
    "customSqlCleanup": {
      "en": "Custom SQL Cleanup",
      "zh": "自定义 SQL 清理"
    },
    "customSqlTip": {
      "en": "Add custom DELETE SQL statements for scheduled cleanup. Only single DELETE statement is allowed per entry.",
      "zh": "添加自定义 DELETE SQL 语句进行定时清理。每条记录仅允许单条 DELETE 语句。"
    },
    "deleteCustomSql": {
      "en": "Delete",
      "zh": "删除"
    },
    "emptyAddressLabel": {
      "en": "Cleanup the empty address before n days",
      "zh": "清理 n 天前空邮件的邮箱地址"
    },
    "inactiveAddressLabel": {
      "en": "Cleanup the inactive address before n days",
      "zh": "清理 n 天前的未活跃地址"
    },
    "mailBoxLabel": {
      "en": "Cleanup the inbox before n days",
      "zh": "清理 n 天前的收件箱"
    },
    "mailUnknowLabel": {
      "en": "Cleanup the unknow mail before n days",
      "zh": "清理 n 天前的无收件人邮件"
    },
    "save": {
      "en": "Save",
      "zh": "保存"
    },
    "saveSuccess": {
      "en": "Save success",
      "zh": "保存成功"
    },
    "sendBoxLabel": {
      "en": "Cleanup the sendbox before n days",
      "zh": "清理 n 天前的发件箱"
    },
    "sqlName": {
      "en": "Name",
      "zh": "名称"
    },
    "sqlNamePlaceholder": {
      "en": "e.g. Clean old logs",
      "zh": "例如: 清理旧日志"
    },
    "sqlPlaceholder": {
      "en": "e.g. DELETE FROM raw_mails WHERE source GLOB '*{'@'}example.com' AND created_at < datetime('now', '-3 day')",
      "zh": "例如: DELETE FROM raw_mails WHERE source GLOB '*{'@'}example.com' AND created_at < datetime('now', '-3 day')"
    },
    "sqlStatement": {
      "en": "SQL Statement (DELETE only)",
      "zh": "SQL 语句 (仅限 DELETE)"
    },
    "tip": {
      "en": "Please input the days",
      "zh": "请输入天数"
    },
    "unboundAddressLabel": {
      "en": "Cleanup the unbound address before n days",
      "zh": "清理 n 天前的未绑定用户地址"
    }
  },
  "views.common.Login": {
    "autoGeneratedName": {
      "en": "Auto-generated name",
      "zh": "自动生成名称"
    },
    "bindUserAddressError": {
      "en": "Error when bind email address to user",
      "zh": "绑定邮箱地址到用户时错误"
    },
    "bindUserInfo": {
      "en": "Logged in user, login without binding email or create new email address will bind to current user",
      "zh": "已登录用户, 登录未绑定邮箱或创建新邮箱地址将绑定到当前用户"
    },
    "credential": {
      "en": "Email Address Credential",
      "zh": "邮箱地址凭据"
    },
    "credentialInput": {
      "en": "Please input the Mail Address Credential",
      "zh": "请输入邮箱地址凭据"
    },
    "credentialLogin": {
      "en": "Credential Login",
      "zh": "凭据登录"
    },
    "email": {
      "en": "Email",
      "zh": "邮箱"
    },
    "emailPasswordRequired": {
      "en": "Email and password are required",
      "zh": "邮箱和密码不能为空"
    },
    "enableRandomSubdomain": {
      "en": "Use Random Subdomain",
      "zh": "启用随机子域名"
    },
    "enableCustomSubdomain": {
      "en": "Use Custom Subdomain",
      "zh": "使用自定义子域名"
    },
    "generateName": {
      "en": "Generate Fake Name",
      "zh": "生成随机名字"
    },
    "getNewEmail": {
      "en": "Create New Email",
      "zh": "创建新邮箱"
    },
    "normalSubdomain": {
      "en": "Normal Domain",
      "zh": "普通域名"
    },
    "getNewEmailTip1": {
      "en": "Please input the email you want to use. only allow: ",
      "zh": "请输入你想要使用的邮箱地址, 只允许: "
    },
    "getNewEmailTip2": {
      "en": "Levaing it blank will generate a random email address.",
      "zh": "留空将会生成一个随机的邮箱地址。"
    },
    "getNewEmailTip3": {
      "en": "You can choose a domain from the dropdown list.",
      "zh": "你可以从下拉列表中选择一个域名。"
    },
    "help": {
      "en": "Help",
      "zh": "帮助"
    },
    "login": {
      "en": "Login",
      "zh": "登录"
    },
    "loginAndBind": {
      "en": "Login and Bind",
      "zh": "登录并绑定"
    },
    "addMailbox": {
      "en": "Add Mailbox",
      "zh": "添加邮箱"
    },
    "ok": {
      "en": "OK",
      "zh": "确定"
    },
    "password": {
      "en": "Password",
      "zh": "密码"
    },
    "passwordLogin": {
      "en": "Password Login",
      "zh": "密码登录"
    },
    "pleaseGetNewEmail": {
      "en": "Ask an administrator to create a mailbox, then add it here with the address password or credential.",
      "zh": "请向管理员申请开通邮箱，然后在此用地址密码或凭据添加。"
    },
    "randomSubdomainTip": {
      "en": "When enabled, the created address will use a random subdomain. Recommended for receiving only. Requires a wildcard MX DNS record on the base domain — see the random subdomain docs.",
      "zh": "启用后，创建出来的地址会自动挂在随机子域名下，建议仅用于收件。需要在基础域名 DNS 中配置通配 MX 记录，详见随机子域名文档。"
    }
  },
  "views.admin.Webhook": {
    "enableAllowList": {
      "en": "Enable Allow List (Restrict webhook access to specific users)",
      "zh": "启用白名单 (限制 webhook 访问权限，只有白名单中的用户可以使用)"
    },
    "manualInputPrompt": {
      "en": "Type and press Enter to add",
      "zh": "输入后按回车键添加"
    },
    "notEnabled": {
      "en": "Webhook is not enabled",
      "zh": "Webhook 未开启"
    },
    "save": {
      "en": "Save",
      "zh": "保存"
    },
    "successTip": {
      "en": "Success",
      "zh": "成功"
    },
    "webhookAllowList": {
      "en": "Webhook Allow List(Enter the mail address that is allowed to use webhook and enter)",
      "zh": "Webhook 白名单(请输入允许使用webhook 的邮箱地址, 回车增加)"
    }
  },
  "views.common.AdminContact": {
    "adminContact": {
      "en": "If you need help, please contact the administrator ({msg})",
      "zh": "如果你需要帮助，请联系管理员 ({msg})"
    }
  },
  "views.common.EnterpriseLogin": {
    "companyNameFallback": {
      "en": "企业邮箱管理平台",
      "zh": "企业邮箱管理平台"
    },
    "brandTitle": {
      "en": "企业邮箱管理平台",
      "zh": "企业邮箱管理平台"
    },
    "brandSubtitle": {
      "en": "A secure, self-hosted mail system on your own domain. Admins provision team mailboxes in seconds.",
      "zh": "安全、自托管的企业邮件系统，基于你自己的域名，由管理员为团队开通邮箱。"
    },
    "brandPoint1": {
      "en": "Admins provision mailboxes on your domain",
      "zh": "在自有域名下由管理员批量开通邮箱"
    },
    "brandPoint2": {
      "en": "Each mailbox has its own retention and access control",
      "zh": "每个邮箱独立的保留期与访问控制"
    },
    "brandPoint3": {
      "en": "Runs entirely on Cloudflare Workers",
      "zh": "完全运行在 Cloudflare Workers 之上"
    },
    "brandFoot": {
      "en": "版权所有 ©2026-2030 企业邮箱转发",
      "zh": "版权所有 ©2026-2030 企业邮箱转发"
    },
    "loginTitle": {
      "en": "Sign in",
      "zh": "登录"
    },
    "loginSubtitle": {
      "en": "Sign in to your mailbox, or open the admin console.",
      "zh": "登录你的邮箱，或进入管理员控制台。"
    },
    "employeeTab": {
      "en": "Mailbox Login",
      "zh": "邮箱登录"
    },
    "adminTab": {
      "en": "Admin",
      "zh": "管理员"
    },
    "emailLabel": {
      "en": "Email address",
      "zh": "邮箱地址"
    },
    "emailPlaceholder": {
      "en": "Enter your email",
      "zh": "请输入邮箱"
    },
    "passwordLabel": {
      "en": "Password",
      "zh": "密码"
    },
    "passwordPlaceholder": {
      "en": "Enter your password",
      "zh": "请输入密码"
    },
    "credentialLabel": {
      "en": "Mailbox credential",
      "zh": "邮箱凭据"
    },
    "credentialPlaceholder": {
      "en": "Paste the JWT credential",
      "zh": "请粘贴 JWT 凭据"
    },
    "usernameLabel": {
      "en": "Username",
      "zh": "用户名"
    },
    "usernamePlaceholder": {
      "en": "Enter admin username",
      "zh": "请输入管理员账号"
    },
    "loginEmail": {
      "en": "Sign in to mailbox",
      "zh": "登录邮箱"
    },
    "loginAdmin": {
      "en": "Open admin console",
      "zh": "进入管理控制台"
    },
    "credentialLogin": {
      "en": "Use credential instead",
      "zh": "改用凭据登录"
    },
    "passwordLogin": {
      "en": "Use password instead",
      "zh": "改用密码登录"
    },
    "adminHint": {
      "en": "Both username and password are verified.",
      "zh": "用户名和密码都会校验。"
    },
    "employeeHint": {
      "en": "Sign in with the mailbox and password assigned by an admin.",
      "zh": "使用管理员分配给你的邮箱与密码登录。"
    },
    "emailPasswordRequired": {
      "en": "Email and password are required",
      "zh": "邮箱和密码不能为空"
    },
    "adminRequired": {
      "en": "Username and password are required",
      "zh": "用户名和密码不能为空"
    },
    "credentialInput": {
      "en": "Please paste the mailbox credential",
      "zh": "请输入邮箱地址凭据"
    },
    "loginFailed": {
      "en": "Login failed",
      "zh": "登录失败"
    }
  },
  "views.admin.SubAdmins": {
    "tip": {
      "en": "Create sub-admin accounts, add quota, change passwords, enable or disable access, and delete accounts.",
      "zh": "开子管理员账号、加积分、改密码、启用/禁用，以及删除。"
    },
    "create": {
      "en": "Create",
      "zh": "开账号"
    },
    "username": {
      "en": "Username",
      "zh": "用户名"
    },
    "password": {
      "en": "Password",
      "zh": "密码"
    },
    "quotaBalance": {
      "en": "Quota",
      "zh": "剩余积分"
    },
    "initialQuota": {
      "en": "Initial quota",
      "zh": "初始积分"
    },
    "enabled": {
      "en": "Enabled",
      "zh": "启用"
    },
    "created_at": {
      "en": "Created At",
      "zh": "创建时间"
    },
    "actions": {
      "en": "Actions",
      "zh": "操作"
    },
    "addQuota": {
      "en": "Add quota",
      "zh": "加积分"
    },
    "changePassword": {
      "en": "Change password",
      "zh": "改密码"
    },
    "newPassword": {
      "en": "New password",
      "zh": "新密码"
    },
    "pleaseInputPassword": {
      "en": "Please enter a new password",
      "zh": "请输入新密码"
    },
    "enable": {
      "en": "Enable",
      "zh": "启用"
    },
    "disable": {
      "en": "Disable",
      "zh": "禁用"
    },
    "enabledOn": {
      "en": "On",
      "zh": "开"
    },
    "enabledOff": {
      "en": "Off",
      "zh": "关"
    },
    "delete": {
      "en": "Delete",
      "zh": "删除"
    },
    "deleteConfirm": {
      "en": "Delete sub-admin {username}?",
      "zh": "确定删除子管理员 {username}？"
    },
    "deleteConfirmTip": {
      "en": "If this sub-admin still owns mailboxes, deletion will be refused.",
      "zh": "若该子管理仍有邮箱，将无法删除。"
    },
    "quotaDelta": {
      "en": "Quota change",
      "zh": "积分变动"
    },
    "quotaDeltaTip": {
      "en": "Use a positive number to add quota.",
      "zh": "正数为充值积分。"
    },
    "pleaseInput": {
      "en": "Please fill in all fields",
      "zh": "请填写完整信息"
    },
    "invalidQuota": {
      "en": "Enter a non-zero quota change",
      "zh": "请输入非零的积分变动"
    },
    "success": {
      "en": "Success",
      "zh": "成功"
    },
    "save": {
      "en": "Save",
      "zh": "保存"
    }
  },
  "views.admin.DomainCreateCosts": {
    "tip": {
      "en": "Cost charged to a sub admin when creating a mailbox on each domain. Default is 1.",
      "zh": "子管理员在各域名下开邮箱时扣除的积分，默认 1。"
    },
    "save": {
      "en": "Save",
      "zh": "保存"
    },
    "success": {
      "en": "Success",
      "zh": "成功"
    },
    "noDomains": {
      "en": "No domains configured",
      "zh": "尚未配置域名"
    }
  },
  "components.UsageGuide": {
    "title": {
      "en": "Quick start",
      "zh": "使用引导"
    },
    "adminStep1": {
      "en": "Create a mailbox in the account list and set expire days.",
      "zh": "在账号列表中开通邮箱，并填写到期天数。"
    },
    "adminStep2": {
      "en": "Give the address and password to the user.",
      "zh": "把邮箱地址和密码发给使用者。"
    },
    "adminStep3": {
      "en": "The user signs in with Mailbox Login, then adds more mailboxes and uses AI extract or JWT credentials as needed.",
      "zh": "使用者用「邮箱登录」进入个人工作台，可添加多个邮箱，并在账户设置中查看 AI 提取与 Agent 凭据用法。"
    },
    "personalStep1": {
      "en": "Sign in with the mailbox and password assigned by an admin.",
      "zh": "使用管理员分配的邮箱地址和密码登录。"
    },
    "personalStep2": {
      "en": "Open Address Manage to add more mailboxes and switch between them. Batch remove only deletes local list items.",
      "zh": "在「地址管理」中添加多个邮箱并切换。批量移除只会从本机列表去掉，不会删服务器上的号。"
    },
    "personalStep3": {
      "en": "Open account settings for JWT credentials, AI extract, Telegram, or the auto-login link /?jwt=.",
      "zh": "在账户设置中查看 JWT 凭据、AI 提取、Telegram，或使用 JWT 深链 /?jwt= 自动登录。"
    }
  }
} as const

type MessageRegistry = typeof MESSAGE_REGISTRY

export type MessageNamespace = keyof MessageRegistry
export type MessageKey<N extends MessageNamespace> = keyof MessageRegistry[N]

export const getMessageSource = <N extends MessageNamespace>(
  namespace: N,
  key: MessageKey<N>,
  locale: 'en' | 'zh',
) => {
  return MESSAGE_REGISTRY[namespace]?.[key]?.[locale]
}
