...
import { requireOwnedAddressByName } from "../admin_auth";
...
export const sendMailbyAdmin = async (c: Context<HonoCustomType>) => {
    ...
    if (from_mail) {
        const denied = await requireOwnedAddressByName(c, from_mail);
...
export const sendMailByBindingAdmin = async (c: Context<HonoCustomType>) => {
    ...
    if (fromMail) {
        const denied = await requireOwnedAddressByName(c, fromMail);
        if (denied) return denied;
    }
