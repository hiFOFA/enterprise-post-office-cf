...
import { getDomainCreateCost, type DomainCreateCosts } from './admin_quota';
...
export const ADDRESS_ACTIVE_SQL =
    `(expires_at IS NULL OR expires_at >= datetime('now'))`;
export const ADDRESS_EXPIRED_SQL =
    `(expires_at IS NOT NULL AND expires_at < datetime('now'))`;

export type AddressOwnerOptions = {
    ownerAdminId?: number | null
    ownerAdminType?: 'main' | 'sub' | null
    expireDays?: number | null
    quota?: { subAdminId: number, cost?: number, costs?: DomainCreateCosts } | null
}
...
export const releaseExpiredAddressByName = async (
    c: Context<HonoCustomType>,
    address: string
): Promise<boolean> => {
    try {
        const row = await c.env.DB.prepare(
            `SELECT id FROM address WHERE name = ? AND ${ADDRESS_EXPIRED_SQL}`
        ).bind(address).first<{ id: number }>();
        if (!row?.id) {
            return false;
        }
        await batchDeleteAddressWithData(c, `id = ${Number(row.id)}`);
        return true;
    } catch (e) {
        const message = (e as Error).message || "";
        if (isMissingColumnError(message, "expires_at")) {
            return false;
        }
        throw e;
    }
...
const buildAddressInsertStatement = (
    c: Context<HonoCustomType>,
    address: string,
    sourceMeta: string | undefined | null,
    owner: AddressOwnerOptions | undefined,
    mode: "full" | "source_meta" | "name"
) => {
    if (mode === "name") {
        return c.env.DB.prepare(`INSERT INTO address(name) VALUES(?)`).bind(address);
    }
    if (mode === "source_meta") {
    ...
    const ownerAdminId = owner?.ownerAdminId ?? null;
    const ownerAdminType = owner?.ownerAdminType ?? null;
    const expireDays = owner?.expireDays ?? null;
    if (expireDays != null) {
        return c.env.DB.prepare(
            `INSERT INTO address(name, source_meta, owner_admin_id, owner_admin_type, expires_at)`
            + ` VALUES(?, ?, ?, ?, datetime('now', ?))`
        ).bind(address, sourceMeta, ownerAdminId, ownerAdminType, `+${expireDays} day`);
    }
    return c.env.DB.prepare(
        `INSERT INTO address(name, source_meta, owner_admin_id, owner_admin_type, expires_at)`
        + ` VALUES(?, ?, ?, ?, NULL)`
    ).bind(address, sourceMeta, ownerAdminId, ownerAdminType);
}

const getD1Changes = (result: { meta?: { changes?: number } } | undefined): number => {
    return Number(result?.meta?.changes ?? 0);
}

const refundSubAdminQuota = async (
    c: Context<HonoCustomType>,
    subAdminId: number,
    cost: number
): Promise<void> => {
    ...
    try {
        await c.env.DB.prepare(
            ...
            + ` WHERE id = ?`
        ).bind(cost, subAdminId).run();
    } catch (e) {
        console.error("Failed to refund sub-admin quota", e);
    }
}

const unbindWebhookByAddress = async (
    c: Context<HonoCustomType>,
    address: string
): Promise<void> => {
    if (!c.env.KV || !address) {
...
export const unbindAddressSideEffects = async (
    c: Context<HonoCustomType>,
    address: string
): Promise<void> => {
    await unbindTelegramByAddress(c, address);
    await unbindWebhookByAddress(c, address);
}

const runQuotaAddressBatch = async (
    c: Context<HonoCustomType>,
    address: string,
    sourceMeta: string | undefined | null,
    owner: AddressOwnerOptions,
    msgs: ReturnType<typeof i18n.getMessagesbyContext>
): Promise<void> => {
    const quota = owner.quota;
    if (!quota) {
        throw new Error(msgs.FailedCreateAddressMsg);
    }
    const cost = typeof quota.cost === "number"
        ? quota.cost
        : getDomainCreateCost(quota.costs, address.split("@")[1] || "");
    // SQLite treats 1/0 as NULL, so a later INSERT would still run. Deduct
    // first and only insert when meta.changes > 0.
    if (cost > 0) {
        const deducted = await c.env.DB.prepare(
            `UPDATE sub_admins SET quota_balance = quota_balance - ?, updated_at = datetime('now')`
            + ` WHERE id = ? AND enabled = 1 AND quota_balance >= ?`
        ).bind(cost, quota.subAdminId, cost).run();
        if (getD1Changes(deducted) === 0) {
            throw new Error(msgs.InsufficientQuotaMsg);
        }
    } else {
        const enabled = await c.env.DB.prepare(
            `SELECT id FROM sub_admins WHERE id = ? AND enabled = 1`
        ).bind(quota.subAdminId).first("id");
        if (!enabled) {
            throw new Error(msgs.FailedCreateAddressMsg);
        }
    }
    const stmts = [
        buildAddressInsertStatement(c, address, sourceMeta, owner, "full"),
    ];
    if (cost > 0) {
        stmts.push(
            c.env.DB.prepare(
                `INSERT INTO sub_admin_quota_ledger (sub_admin_id, delta, reason, address_id)`
                + ` VALUES (?, ?, 'create_address', last_insert_rowid())`
            ).bind(quota.subAdminId, -cost)
        );
    }
    try {
        const results = await c.env.DB.batch(stmts);
        if (getD1Changes(results[0]) === 0) {
            await refundSubAdminQuota(c, quota.subAdminId, cost);
            throw new Error(msgs.FailedCreateAddressMsg);
        }
    } catch (e) {
        if ((e as Error).message === msgs.FailedCreateAddressMsg) {
            throw e;
        }
        await refundSubAdminQuota(c, quota.subAdminId, cost);
        throw e;
...
const insertAddressRecord = async (
    ...
    const tryInsert = async (): Promise<void> => {
        if (owner?.quota) {
            await runQuotaAddressBatch(c, address, sourceMeta, owner, msgs);
            return;
        }
        try {
            const result = await buildAddressInsertStatement(
                c, address, sourceMeta, owner, owner ? "full" : "source_meta"
            ).run();
            if (!result.success) {
                throw new Error(msgs.FailedCreateAddressMsg)
            }
            ...
            const message = (e as Error).message || "";
            if (owner && (
                isMissingColumnError(message, "owner_admin_id")
                || isMissingColumnError(message, "owner_admin_type")
                || isMissingColumnError(message, "expires_at")
            )) {
                const result = await buildAddressInsertStatement(
                    c, address, sourceMeta, owner, "source_meta"
                ).run();
                if (!result.success) {
                    throw new Error(msgs.FailedCreateAddressMsg)
                }
            ...
            if (message && message.includes("source_meta")) {
                const result = await buildAddressInsertStatement(
                    c, address, sourceMeta, owner, "name"
                ).run();
                if (!result.success) {
                    throw new Error(msgs.FailedCreateAddressMsg)
                }
    ...
    try {
        ...
        if (isUniqueConstraintError(message)) {
            const released = await releaseExpiredAddressByName(c, address);
            if (released) {
                await tryInsert();
                return;
            }
            throw new Error(msgs.AddressAlreadyRegisteredMsg);
        }
        throw e;
    }
}
...
export const newAddress = async (
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        try {
            await insertAddressRecord(c, address, sourceMeta, msgs, owner ? {
                ...owner,
                quota: owner.quota ? {
                    subAdminId: owner.quota.subAdminId,
                    cost: typeof owner.quota.cost === "number"
                        ? owner.quota.cost
                        : getDomainCreateCost(owner.quota.costs, addressDomain),
                    costs: owner.quota.costs,
                } : null,
            } : undefined);
            ...
            const message = (e as Error).message;
            if (message && (message.includes("UNIQUE") || message === msgs.AddressAlreadyRegisteredMsg)) {
                if (enableRandomSubdomain && attempt < maxAttempts - 1) {
                    continue;
                }
                throw new Error(msgs.AddressAlreadyRegisteredMsg)
            }
            if (message === msgs.InsufficientQuotaMsg) {
                throw e;
            }
            throw new Error(message || msgs.FailedCreateAddressMsg)
        }
    }
        ...
        case "expiredAddress":
            try {
                await batchDeleteAddressWithData(
                    c,
                    `id IN (`
                    + `SELECT id FROM address WHERE ${ADDRESS_EXPIRED_SQL} `
                    + `ORDER BY expires_at, id LIMIT ${cleanupBatchSize})`
                )
            } catch (e) {
                const message = (e as Error).message || "";
                if (message.includes("expires_at") || message.includes("no such column")) {
                    console.log("Skip expiredAddress cleanup: expires_at column missing");
                    break;
                }
                throw e;
            }
            break;
        case "unboundAddress": {
            const boundCount = await c.env.DB.prepare(
                `SELECT COUNT(*) as count FROM users_address`
            ).first<number>("count");
            if (!boundCount) {
                console.log("Skip unboundAddress cleanup: no users_address bindings");
                break;
            }
            try {
                await batchDeleteAddressWithData(
                    c,
                    `id NOT IN (SELECT address_id FROM users_address)`
                    + ` AND owner_admin_id IS NULL`
                    + ` AND (owner_admin_type IS NULL OR owner_admin_type = '')`
                    + ` AND created_at < datetime('now', '-${cleanDays} day')`
                )
            } catch (e) {
                const message = (e as Error).message || "";
                if (message.includes("owner_admin_id") || message.includes("no such column")) {
                    console.log("Skip unboundAddress cleanup: owner_admin_id column missing");
                    break;
                }
                throw e;
            }
...
const batchDeleteAddressWithData = async (
    ...
    for (const row of results || []) {
        if (row.name) {
            await unbindAddressSideEffects(c, row.name);
        }
    }
    await c.env.DB.prepare(
...
export const deleteAddressWithData = async (
    ...
    await unbindAddressSideEffects(c, address);
