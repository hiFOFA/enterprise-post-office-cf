...
async function email(message: ForwardableEmailMessage, env: Bindings, ctx: ExecutionContext) {
    ...
    // reject unknown or expired addresses — do not insert raw_mails
    try {
        const db_address_id = await env.DB.prepare(
            `SELECT id FROM address WHERE name = ? AND (expires_at IS NULL OR expires_at >= datetime('now'))`
        ).bind(toAddress).first("id");
        if (!db_address_id) {
            message.setReject("Unknown or expired address");
            console.log(`Unknown or expired address mail from ${message.from} to ${toAddress}`);
            return;
        }
    } catch (error) {
        const errMsg = String(error);
        if (errMsg.includes("expires_at") || errMsg.includes("no such column")) {
            try {
                const db_address_id = await env.DB.prepare(
                ...
                console.error("check address existence error", fallbackError);
                message.setReject("Unknown address");
                return;
            }
        } else {
            console.error("check address expiry error", error);
            message.setReject("Unknown or expired address");
            return;
        }
    }
