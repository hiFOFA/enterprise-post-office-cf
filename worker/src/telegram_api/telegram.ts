...
import { bindTelegramAddress, deleteTelegramAddress, jwtListToAddressData, unbindTelegramAddress, unbindTelegramByAddress } from "./common";
...
export const getTelegramCommands = (c: Context<HonoCustomType>) => {
    let commands = COMMANDS.filter(cmd => cmd.command !== "new");
    if (!getBooleanValue(c.env.TG_ALLOW_USER_LANG)) {
        commands = commands.filter(cmd => cmd.command !== "lang");
    }
    return commands;
}
...
export function newTelegramBot(c: Context<HonoCustomType>, token: string): Telegraf {
    ...
    bot.command("new", async (ctx: TgContext) => {
        const msgs = await getTgMessages(c, ctx);
        return await ctx.reply(msgs.TgNewAddressDisabledMsg);
    });
